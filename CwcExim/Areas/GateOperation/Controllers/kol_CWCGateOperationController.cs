...
{
    public class kol_CWCGateOperationController : BaseController
    {
        ...
        public ActionResult EditEntryThroughGateExport(int EntryId)
        {
            ...
            if (EntryId > 0)
            {
                ObjETR.GetEntryThroughGate_Kolkata(EntryId);
                if (ObjETR.DBResponse.Data != null)
                {
        ...
        public ActionResult AddEditEntryThroughGate(EntryThroughGate ObjEntryThroughGate)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                ObjEntryThroughGateRepositories.AddEditEntryThroughGate_Kolkata(ObjEntryThroughGate);
                // ModelState.Clear();
        ...
        public ActionResult ViewEntryThroughGate(int EntryId)
        {
            ...
            {
                //EntryThroughGateRepository ObjGOR = new EntryThroughGateRepository();
                ObjEtGateR.GetEntryThroughGate_Kolkata(EntryId);
        ...
        #region LWB For WorkSlipSlip
        [HttpGet]
        public ActionResult CreateLWBForWS()
        {
            return PartialView();
        }

        [HttpGet]
        public JsonResult GetContainersForLWB(string GateInDate)
        {
            EntryThroughGateRepository ObjETR = new EntryThroughGateRepository();
            ObjETR.GetCFSCodeListForLWB(GateInDate);
            return Json(ObjETR.DBResponse, JsonRequestBehavior.AllowGet);

        }

        [HttpGet]
        public JsonResult GetLWBEntrydetails(int Id=0)
        {
            EntryThroughGateRepository ObjETR = new EntryThroughGateRepository();
            ObjETR.GetLWBEntrydetails(Id);
            return Json(ObjETR.DBResponse, JsonRequestBehavior.AllowGet);

        }
        [HttpPost]
        public JsonResult AddEditContainersForLWB(LWBContainer obj)
        {
            EntryThroughGateRepository ObjETR = new EntryThroughGateRepository();
            Login ObjLogin = (Login)Session["LoginUser"];
            ObjETR.AddEditLwbContainer(obj, ObjLogin.Uid);
            return Json(ObjETR.DBResponse, JsonRequestBehavior.AllowGet);
        }
        #endregion
    }
