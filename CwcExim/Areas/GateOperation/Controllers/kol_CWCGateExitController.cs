...
{
    public class kol_CWCGateExitController : BaseController
    {
                ...
                if (objETGR.DBResponse.Status == 1)
                {
                    ...
                    //{
                        ...
                        arrExitThroughGateDetailsList.Where(y=>y.OperationType=="Import").GroupBy(o=>o.ShippingLineId).ToList().ForEach(item => {
                            ...
                            var Sline = arrExitThroughGateDetailsList.Where(o => o.ShippingLineId.ToString() == item.Key.ToString()).Select(o => o.ShippingLine).FirstOrDefault();
                            //var sid = item.ShippingLineId.ToString();
                            if (Sline.ToUpper().Equals("PIL")) {
                                //For PIL
                                //SendExitMailPIL(item.Key.ToString());
                    ...
                    //For PIL Only
                    /*arrExitThroughGateDetailsList.Where(y => y.OperationType == "Export").GroupBy(o => o.ShippingLineId).ToList().ForEach(item => {
        ...
        [NonAction]
        public int SendExitMailPIL(string ShippingLineId)//string ContainerNo
        {
            try
            {
                string message = "";
                ...
                string CuurDate = DateTime.Now.ToString("MM/dd/yyyy").Replace("/", "");
                ExitThroughGateRepository objExit = new ExitThroughGateRepository();
                objExit.GetDetailsForGateExitMailPIL(ShippingLineId);
                if (objExit.DBResponse.Data != null)
                {
                    //EmailDataModel ObjEmailDataModel = new EmailDataModel();
                    //ObjEmailDataModel.ReceiverEmail = ((EntryThroughGateMail)objExit.DBResponse.Data).Email;
                    var mailTo = ((EntryThroughGateMailPIL)objExit.DBResponse.Data).Email.Replace(" ", "").Split(',');
                    mailTo = mailTo.Where(x => !string.IsNullOrEmpty(x)).ToArray();
                    for (int j = 0; j < mailTo.Length; j++)
                    {
                        mailTo[j].Trim();
                    ...
                    var FileName = ((EntryThroughGateMailPIL)objExit.DBResponse.Data).FileName;
                    //ObjEmailDataModel.Subject = "Test Subject";
                    //ObjEmailDataModel.MailBody = "Exit Through Gate";
                    //List<string> containerList = new List<string>();
                    var excelData = ((EntryThroughGateMailPIL)objExit.DBResponse.Data).lstExcelData;
                    var excelString = string.Empty;
                    excelData.ToList().ForEach(item => {
