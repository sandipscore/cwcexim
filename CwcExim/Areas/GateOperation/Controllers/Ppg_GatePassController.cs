...
namespace CwcExim.Areas.GateOperation.Controllers
{
    ...
    public class Ppg_GatePassController : BaseController
    {
        ...
        {
            PpgGatePassRepository objGPR = new PpgGatePassRepository();
        ...
        {
            string html = "";
            string Vessel = "", Voyage = "", Rotation = "", LineNo = "", NoOfUnits = "", Weight = "", CargoType = "", Location = "", PortOfDispatch="",  EntryDate="", ICDCode="", ShippingLineNo="", CargoDescription="", OBLNO="", IGMNo="", InDate="", DestuffingDate="",CustomSealNo="",InvoiceNo="",IssueSlipNo="",IssueSlipDate="",CreatedTime="",InvoiceType="",CFSCode="", ElwbTareWeight = "", ElwbCargoWeight = "";
         ...
         objGP.lstDet.Select(x => new { CargoDescription = x.CargoDescription }).Distinct().ToList().ForEach(item =>
            ...
            ElwbTareWeight = ((objGP.lstDet.Sum(x => x.ElwbTareWeight) > 0) ? objGP.lstDet.Sum(x => x.ElwbTareWeight).ToString() : "");
            ElwbCargoWeight = ((objGP.lstDet.Sum(x => x.ElwbCargoWeight) > 0) ? objGP.lstDet.Sum(x => x.ElwbCargoWeight).ToString() : "");
            ...
            html += "<tr><th cellpadding='10' align='left' colspan='6' width='50%' style='overflow:hidden; word-wrap:break-word; border-bottom:1px solid #000; border-right:1px solid #000;'>Tare Weight (ELWB) : <span style='font-weight:normal;'>" + ElwbTareWeight + "</span></th>";
            html += "<th cellpadding='10' align='left' colspan='6' width='50%' style='overflow:hidden; word-wrap:break-word; border-bottom:1px solid #000;'>Cargo Weight (ELWB) : <span style='font-weight:normal;'>" + ElwbCargoWeight + "</span></th></tr>";
        ...
        public JsonResult CancelGatePass(int GatePassId)
        {
            if (GatePassId > 0)
            {
                PpgGatePassRepository objGP = new PpgGatePassRepository();
        ...
        public ActionResult ExtendGatePass()
        {
            ...
            PpgGatePassRepository ObjGOR = new PpgGatePassRepository();
        ...
        public async Task<JsonResult> SendDP(int GatePassId = 0)
        {
            try
            {
                int k = 0;
                int j = 1;
                PpgGatePassRepository ObjER = new PpgGatePassRepository();
                ...
                if (ObjER.DBResponse.Status == 1)
                {
                    ...
                    foreach (DataRow dr in ds.Tables[7].Rows)
                    {
                        ...
                        PpgGatePassRepository objExport = new PpgGatePassRepository();
