...
{
    public class kdl_CWCGateExitController : Controller
    {

        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        // GET: GateOperation/kdl_CWCGateExit
        #region Exit Through Gate kandla
   ...
   [HttpGet]
        ...
        public JsonResult GetContainerForGatePass(int GPId)
        {
            ...
            List<containerAgainstGp> LstcontainerAgainstGp = new List<containerAgainstGp>();

            ObjGOR.ContainerForGAtePass(GPId);//, objLogin.Uid
            if (ObjGOR.DBResponse.Data != null)
            {
                LstcontainerAgainstGp = (List<containerAgainstGp>)ObjGOR.DBResponse.Data;
        ...
        {
            if (objETG.Module == "IMPDeli" || objETG.Module == "IMPYard" || objETG.Module == "EC" || objETG.Module == "ECGodn" || objETG.Module == "BNDadv" || objETG.Module == "BND" || objETG.Module == "ECGateIn" || objETG.Module == "ECGateOut" || objETG.Module == "IMPDest" || objETG.Module == "ECGodn" || objETG.Module == "ECOut" || objETG.Module == "ECYard")
            {
        ...
        [HttpGet]
        public ActionResult CreateExitThroughGatePending()
        {
            ExitThroughGateRepository ObjETR = new ExitThroughGateRepository();
            ExitThroughGateHeader objExitThroughGateHeader = new ExitThroughGateHeader();
            ObjETR.GetTime();
        ...
        [HttpGet]
        public ActionResult CreateContGateExit(string date = "")
        {
            ExitThroughGateRepository ObjETR = new ExitThroughGateRepository();
            Kdl_GateEntryExitCBT ObjCBT = new Kdl_GateEntryExitCBT();
            ExitThroughGateHeader objExitThroughGateHeader = new ExitThroughGateHeader();
            ObjETR.GetTime();
            if (ObjETR.DBResponse.Data != null)
            {
            ...
            ObjETR.GetKdlCONTList();
            if (ObjETR.DBResponse.Data != null)
            {

                Lstcontainer = (List<Kdl_CBTList>)ObjETR.DBResponse.Data;
                ViewBag.Lstcontainer = Lstcontainer;
            }
            else
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddEditExitThroughKdlGateCBT(Kdl_GateEntryExitCBT objETG)
        {
            if (ModelState.IsValid)
            {
                ExitThroughGateRepository objETGR = new ExitThroughGateRepository();

                objETGR.AddEditExitThroughKdlGateCBT(objETG, ((Login)(Session["LoginUser"])).Uid);
        ...
        [HttpGet]
        public ActionResult GetAllCBTExitThroughKdlGate()
        {
            ExitThroughGateRepository ETGR = new ExitThroughGateRepository();
            ETGR.GetAllCBTExitThroughKdlGate();
            List<Kdl_GateEntryExitCBT> ListExitThroughGateHeader = new List<Kdl_GateEntryExitCBT>();

            if (ETGR.DBResponse.Data != null)
            {
                ListExitThroughGateHeader = (List<Kdl_GateEntryExitCBT>)ETGR.DBResponse.Data;
            }
            return PartialView("ContGateExitList", ListExitThroughGateHeader);
        ...
        public JsonResult SendDT(int ExitId = 0)
        {
            int k = 0;
            int j = 1;
        ...
        public ActionResult CreateUserIdStatus(string date = "")
        {
            ExitThroughGateRepository ObjETR = new ExitThroughGateRepository();
