        ...
        $scope.CalculateCharges = function () {
            ...
            if (ContList.length > 0 && $scope.IsCalculated == 0) {
                if ($scope.PartyId == 0) {
                    ...
                    $scope.TaxType = $('#Tax').is(':checked') ? 'Tax' : 'Bill';
                    BttContService.CalculateCharges($scope.InvoiceDate, $scope.PartyId, $scope.PayeeId, $scope.TaxType, ContList, $scope.CasualLabour, $scope.InvoiceId,$scope.ExportUnder).then(function (data) {
