(function () {
    ...
    controller('Dnd_CargoShiftingCtrl', function ($scope, Dnd_CargoShiftingService) {
        $scope.InvoiceNo = "";
        $scope.shipbills = [];
        $scope.Message = '';
        ...
        $scope.GetShipBillDetails = function () {
            ...
            if ($scope.ShiftingType == 1 && $scope.ShippingLineIdF > 0) {
                Dnd_CargoShiftingService.GetShipBillDetails($scope.ShippingLineIdF, $scope.ShiftingType, $scope.GodownIdF).then(function (res) {
                    if (res.data == 0) {
                        $scope.shipbills = [];
                        $scope.InvoiceObj = [];
                        $scope.IsCalculate = false;
        ...
        $scope.GetCargoShiftingInvoice = function () {
            ...
            Dnd_CargoShiftingService.GetCargoShiftingInvoice(0, $scope.ShiftngDate, $scope.ShippingLineIdF, TaxType, shipbills, $scope.PartyId).then(function (res) {
        ...
        $scope.AddEditCargoShifting = function () {
            ...
            $scope.ShiftingNo = $scope.ShiftingNo;
            $scope.ShiftngDate = $scope.ShiftngDate;
            var conf = confirm('Are you sure you want to Save?');
            if (conf == true) {
                $('#BtnMOveCargoShp').attr('disabled', true);

                var shipbills = $scope.shipbills.filter(x=>x.IsChecked == true);
                Dnd_CargoShiftingService.AddEditCargoShifting($scope.InvoiceObj, shipbills, $scope.GodownIdF, $scope.GodownIdT, $scope.ShippingLineIdT, $scope.ShiftingType, $scope.ShippingLineIdF).then(function (res) {
                    ...
                    if (res.data.Data != null) {
                        ...
                        $scope.disMove = true;
                        //$scope.LoadCargoShift();
                        setTimeout(LoadCargoShifting, 3000);
        ...
        $scope.GetShippingLine = function () {
            ...
            debugger;
            Dnd_CargoShiftingService.GetShippingLineforCargo().then(function (res) {
                debugger;
        ...
        $scope.GetParty = function () {
            ...
            debugger;
            Dnd_CargoShiftingService.GetPartyforCargo().then(function (res) {
                debugger;
        ...
        function LoadCargoShifting() {
            $('#DivBody').load('/Export/Dnd_CWCExport/CreateCargoShifting?type=""');
        }

        $scope.GetGodown = function () {

            debugger;
            Dnd_CargoShiftingService.GetGodownforCargo().then(function (res) {
                debugger;
        ...
        $scope.GetGodownTo = function () {
            ...
            debugger;
            Dnd_CargoShiftingService.GetGodownforToCargo().then(function (res) {
                debugger;
