(function () {
    ...
    controller('WFLD_AmendmentCtrl', function ($scope, WFLD_AmendmentService) {
        $scope.InvoiceNo = '';
        $scope.InvoiceId = 0;
        $scope.InvoiceDate = '';
        $scope.tabInvoiceNo = '';
        ...
        $scope.searchCommodity = function () {
            ...
            WFLD_AmendmentService.loadMoreCommodity($scope.partyCode, 0).then(function (res) {
                debugger;
        ...
        $scope.LoadMoreCommodity = function () {
            ...
            WFLD_AmendmentService.loadMoreCommodity($scope.partyCode, page).then(function (res) {
                debugger;
                if (res.data.Status == 1) {
        ...
        $scope.LoadMoreCommoditytab = function () {
            ...
            WFLD_AmendmentService.loadMoreCommodity($scope.tabCommodityBox, $scope.tabPageFlag).then(function (res) {
                debugger;
        ...
        $scope.GetCBDetailsBySbIdDate = function (SbNo, SbDate) {
            ...
            $scope.OldNoOfVehicle = 0;
            $scope.OldCargoInvoiceNo='';
            $scope.OldCargoInvoiceDate='';
            $scope.UnReservedCBM = 0;
            $scope.ReservedCBM = 0;

            $('#SBNoModal').modal('hide');
            WFLD_AmendmentService.GetSBDetails(SbNo, SbDate).then(function (res) {
                debugger;
                ...
                if ($scope.SBDetails.length > 0) {
                    ...
                    $scope.OldNoOfVehicle = $scope.SBDetails[0].VehicleNo;
                    $scope.OldCargoInvoiceNo = $scope.SBDetails[0].CCINInvoiceNo;
                    $scope.OldCargoInvoiceDate = $scope.SBDetails[0].CCINInvoiceDate;
                    $scope.NoOfGround = $scope.SBDetails[0].NoofGround;
                    $scope.UnReservedCBM = $scope.SBDetails[0].UnReservedCBM;
                    $scope.ReservedCBM = $scope.SBDetails[0].ReservedCBM;
            ...
            if ($scope.CCINId != "") {
                ...
                if ($scope.OldData.length > 0) {
                    ...
                    $scope.OldData.push({
                        ...
                        'OldNoOfVehicle': $scope.OldNoOfVehicle,
                        'OldCargoInvoiceNo': $scope.OldCargoInvoiceNo,
                        'OldCargoInvoiceDate': $scope.OldCargoInvoiceDate,
                        'OldUnReservedCBM': $scope.UnReservedCBM,
                        'OldReservedCBM': $scope.ReservedCBM
                    });
                    $scope.ShipBillDate = "";
                    $scope.txtShipBillNo = "";
                    $scope.Party = "";
        ...
        $scope.PushValueArrayNewInfo = function () {
            if ($scope.txtNewPartyName != '' && $scope.txtNewShipBillNo != '' && $scope.txtNewShipBillDate != '' && $scope.txtWeight > 0 && $scope.txtPkg > 0 && $scope.txtFOB > 0 ) {
                ...
                $scope.NewData.push(
             {
                 ...
                 'NewInfoUnReservedCBM': $scope.txtUnReservedCBM,
                 'NewInfoReservedCBM': $scope.txtReservedCBM,
                 'NewInfoFOB': $scope.txtFOB,
                 'NewNoOfVehicle': $scope.txtVehicleNo,
                 'NewCargoInvoiceNo': $scope.txtCargoInvoice,
                 'NewCargoInvoiceDate': $scope.txtCargiInvoiceDate
             }
             );
                $scope.txtNewPartyName = '';
                $scope.NewPartyId = '';
                ...
                $scope.txtFOB = '';
                $scope.txtVehicleNo = '';
                $scope.txtCargoInvoice = '';
                $scope.txtCargiInvoiceDate = '';
                $scope.txtUnReservedCBM = '';
                $scope.txtReservedCBM = '';
            ...
            if ($scope.AmendDate == '' || $scope.AmendDate == null || $scope.AmendDate == 'undefined') {
                ...
                if ($scope.OldData.length == 0) {
                    ...
                    if (confirm('Are you sure to save?')) {
                        ...
                        WFLD_AmendmentService.saveAmendment($scope.OldData, $scope.NewData, $scope.AmendDate, $scope.AmendmentNO, $scope.InvoiceId, $scope.InvoiceNo, $scope.InvoiceDate, $scope.FlagMerger,  $scope.NoOfGround).then(function (res) {
                            ...
                            if (res.data != '') {
                                ...
                                if (res.data.Status != 0) {
                                    ...
                                    $scope.InvoiceNo = '';
                                    $scope.InvoiceDate = '';
                                    setTimeout(function () { $('#DivBody').load('/Export/WFLD_CWCExport/Amendment'); }, 5000);
                                }
                                else
                                {
        ...
        $scope.ViewAmendDetails = function (AmendNo) {
            $scope.OldData = [];
            $scope.NewData = [];
            WFLD_AmendmentService.GetAmendDetailsByAmendNo(AmendNo).then(function (res) {
                //console.log(res.data);
        ...
        $scope.tabGetCBDetailsBySbIdDate = function (SbNo, SbDate) {
            ...
            WFLD_AmendmentService.GetSBDetailsByAmend(SbNo, SbDate).then(function (res) {
                debugger;
                $scope.SBDetails = res.data;
                if ($scope.SBDetails.length > 0) {
                    ...
                    $scope.tabtxtCCINNo = $scope.SBDetails[0].CCINNO;
                    ...
                    $scope.txttabOldWeight = $scope.SBDetails[0].Weight;
                    $scope.txttabOldPkg = $scope.SBDetails[0].Pkg;
                    $scope.txttabOldFOB = $scope.SBDetails[0].FOBValue;

                    $scope.tabtxtGrossWeight = $scope.SBDetails[0].Weight;
                    $scope.tabtxtTotalPkg = $scope.SBDetails[0].Pkg;
                    $scope.tabOLDFOB = $scope.SBDetails[0].FOBValue;

                    $scope.tabtxtCHAName = $scope.SBDetails[0].CHAName;
        ...
        $scope.ResetAlltab = function () {
            ...
            $('#DivBody').load('/Export/WFLD_CWCExport/Amendment');
        }
        ...
        $scope.SubmitTabData = function () {
            ...
            if ($scope.ValidationTab()) {
                $scope.vmData =
                {
                    ...
                    'SBType': document.getElementById("NewSBType").value == "Baggage" ? 1 : document.getElementById("NewSBType").value == "Duty Free Goods" ? 2 : 3,
                    ...
                    'OldWeight': $scope.txttabOldWeight,
                    'OldPkg': $scope.txttabOldPkg,
                    'OldFOB': $scope.txttabOldFOB,
                    'CHAName': $scope.tabtxtCHAName

                };

                if (confirm('Are you sure to save?')) {
                    $('#btnSavee').attr("disabled", true);
                    WFLD_AmendmentService.savetabAmendment($scope.vmData).then(function (res) {
                        //console.log(res.data);
                        debugger;
                        if (res.data.Status == '1') {
                            debugger;
                            ...
                            //$scope.GetAmendList();
                            setTimeout(function () { $('#DivBody').load('/Export/WFLD_CWCExport/Amendment'); }, 500);
                        }
                        else {
        ...
        $scope.GetAllCommodity = function () {
            debugger;
            WFLD_AmendmentService.GetAllCommodityName('', $scope.PageFlag).then(function (res) {
                debugger;
                if (res.data != '') {
                    $scope.ListOfCommodityForAmendment = res.data.Data;
                }
        ...
        $scope.tabGetAllCommodity = function () {
            debugger;
            WFLD_AmendmentService.GetAllCommodityName('', $scope.tabPageFlag).then(function (res) {
                debugger;
                if (res.data != '') {
                    $scope.tabListOfCommodityForAmendment = res.data.Data;
                }
        ...
        $scope.GetAmendList = function () {
            ...
            WFLD_AmendmentService.GetAmendLst().then(function (res) {
                debugger;
