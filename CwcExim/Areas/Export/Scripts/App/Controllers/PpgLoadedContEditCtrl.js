(function () {
    ...
    controller('PpgLoadedContInvoiceEditCtrl', function ($scope, PpgLoadedContInvoiceEditService) {
        $scope.InvoiceNo = "";
        $scope.conatiners = [];
        $scope.Message = '';
        ...
        $scope.GetInvNo = function () {
            ...
            debugger;
            PpgLoadedContInvoiceEditService.GetAppNoForYard("EXPLod").then(function (res) {
                debugger;
        ...
        $scope.SelectInvoice = function (obj) {
            ...
            $('.modalloader').show();
            PpgLoadedContInvoiceEditService.GetYardInvoiceDetails($scope.InvoiceId).then(function (res) {
                $('.modalloader').hide();
        ...
        $scope.ContainerSelect = function () {
            ...
            if ($('#InvoiceNo').val() == '') {
                ...
                $('.modalloader').show();
                if (c > 0) {
                    PpgLoadedContInvoiceEditService.ContainerSelect($scope.InvoiceId, $('#InvoiceDate').val(), $scope.StuffingReqId, $scope.TaxType, $scope.PartyId, $scope.PayeeId, $scope.conatiners).then(function (res) {
                        $('.modalloader').hide();
        ...
        $scope.SubmitInvoice = function () {
            ...
            if ($scope.StuffingReqId == 0 || $scope.StuffingReqId == '' || $scope.StuffingReqId == null) {
                $scope.Message = "Select Loaded Container Request Id";
                return false;
            }
            ...
            if (confirm('Are you sure to Update this Invoice?')) {
                ...
                PpgLoadedContInvoiceEditService.GenerateInvoice($scope.InvoiceObj).then(function (res) {
