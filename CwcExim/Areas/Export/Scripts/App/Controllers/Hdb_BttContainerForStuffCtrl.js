(function () {
    ...
    .controller('BttContCtrlForStuffed', function ($scope, BttContServiceForStuffed) {
        ...
        $scope.CalculateCharges = function () {
            ...
            if (ContList.length > 0 && $scope.IsCalculated == 0) {
                if ($scope.PartyId == 0) {
                    ...
                    $scope.TaxType = $('#Tax').is(':checked') ? 'Tax' : 'Bill';
                    BttContServiceForStuffed.CalculateCharges($scope.InvoiceDate, $scope.PartyId, $scope.PayeeId, $scope.TaxType, ContList, $scope.CasualLabour, $scope.InvoiceId, $scope.ExportUnder, $('#IsPrinting').val(), $('#NoOfPrint').val()).then(function (data) {
                        ...
                        $('#IsPrinting').prop('disabled', true);
                        $('#NoOfPrint').prop('disabled', true);
        ...
        $scope.AddEditBTTContPS = function () {
            ...
            if (Conf == true) {
                ...
                BttContServiceForStuffed.AddEditBTTContPS($scope.InvoiceObj).then(function (data) {
        ...
        $scope.LoadParty = function () {
            $scope.Page = 0;
            BttContServiceForStuffed.LoadPartyList($scope.Page).then(function (data) {
                if (data.data.Status == 1) {
        ...
        $scope.SearchPartyByPartyCode = function () {
            debugger;
            $scope.Page = 0;
            BttContServiceForStuffed.SearchPartyNameByPartyCode($scope.PartyCode).then(function (data) {
                if (data.data.Status == 1) {
                    $scope.PartyList = data.data.Data.lstParty;
                    $scope.State = data.data.Data.State;
                }
        ...
        $scope.LoadPayer = function () {
            $scope.PayerPage = 0;
            BttContServiceForStuffed.LoadPayerList($scope.PayerPage).then(function (data) {
                console.log(data);
                if (data.data.Status == 1) {
        ...
        $scope.SearchPayerByPayerCode = function () {
            debugger;
            $scope.PayerPage = 0;
            BttContServiceForStuffed.SearchPayerByPayerCode($scope.PayerCode).then(function (data) {
                if (data.data.Status == 1) {
                    $scope.PayerList = data.data.Data.lstPayer;
                    $scope.StatePayer = data.data.Data.StatePayer;
                }
