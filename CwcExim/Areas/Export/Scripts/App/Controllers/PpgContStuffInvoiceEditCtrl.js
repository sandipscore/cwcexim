(function () {
    ...
    controller('PpgContStuffInvoiceEditCtrl', function ($scope, PpgContStuffInvoiceEditService) {
        $scope.InvoiceNo = "";
        $scope.conatiners = [];
        $scope.Message = '';
        ...
        $scope.GetInvNo = function () {
            ...
            debugger;
            PpgContStuffInvoiceEditService.GetAppNoForYard("EXPCSGRE").then(function (res) {
                debugger;
        ...
        $scope.SelectInvoice = function (obj) {
            ...
            $('.modalloader').show();
            PpgContStuffInvoiceEditService.GetYardInvoiceDetails($scope.InvoiceId).then(function (res) {
                $('.modalloader').hide();
                $scope.conatiners = res.data.Containers;
                $scope.InvoiceObj = res.data.Data;
               
                $scope.TaxType = $scope.InvoiceObj.InvoiceType;
                $scope.InvoiceObj.lstChargs = $scope.InvoiceObj.lstPostPaymentChrg;
                $scope.InvoiceId = $scope.InvoiceObj.InvoiceId;
                $('#InvoiceDate').val($scope.InvoiceObj.DeliveryDate);
                $scope.InvoiceObj.GRETotalAmount = $scope.InvoiceObj.AllTotal;
             ...
             //   $scope.MoveToId = $scope.InvoiceObj.MoveToId;
                $scope.StuffReqId = $scope.InvoiceObj.ContainerStuffingId;
        ...
        $scope.ContainerSelect = function () {
            ...
            if (c > 0) {
                PpgContStuffInvoiceEditService.ContainerSelect($scope.InvoiceId, $('#InvoiceDate').val(), $scope.StuffingReqId, $scope.TaxType, $scope.conatiners).then(function (res) {
        ...
        $scope.GetInvoiceCharges = function () {
            ...
            if ($('#InvoiceNo').val() == '') {
                ...
                $('.modalloader').show();
                PpgContStuffInvoiceEditService.GetInvoiceCharges($scope.StuffingReqDate, $scope.conatiners, $scope.PartyId, $scope.StuffReqId, $scope.InvoiceId).then(function (res) {
                    debugger;
                    $('.modalloader').hide();
                    $scope.InvoiceObj = res.data;
                    $scope.InvoiceObj.RequestNo = $scope.StuffingReqNo;

                    $scope.InvoiceObj.lstPostPaymentChrg = $scope.InvoiceObj.lstChargs;
                    $scope.InvoiceObj.PaymentMode = $scope.PaymentMode;
                    $scope.InvoiceObj.Module = "EXPCSGRE";
                    $scope.InvoiceObj.AllTotal = $scope.InvoiceObj.GRETotalAmount;
                    $scope.InvoiceObj.InvoiceAmt = $scope.InvoiceObj.GRETotalAmount;
                    $scope.InvoiceObj.TotalCGST = $scope.InvoiceObj.GRECGSTCharge;
                    $scope.InvoiceObj.TotalSGST = $scope.InvoiceObj.GRESGSTCharge;
                    $scope.InvoiceObj.TotalIGST = $scope.InvoiceObj.GREIGSTCharge;
                    $scope.InvoiceObj.TotalAmt = $scope.InvoiceObj.GRECharge;
                    $scope.InvoiceObj.RoundUp = 0;
                    $scope.InvoiceObj.lstOperationCFSCodeWiseAmount = $scope.InvoiceObj.GREOperationCFSCodeWiseAmtLst
                    $scope.IsContSelected = true;
                    console.log($scope.InvoiceObj);
                    if ($scope.Rights.CanEdit == 1) {
                        $('#btnSave').removeAttr("disabled");
        ...
        $scope.SubmitInvoice = function () {
            ...
            if (confirm('Are you sure to Update this Invoice?')) {
                ...
                PpgContStuffInvoiceEditService.GenerateInvoice($scope.InvoiceObj).then(function (res) {
