(function () {
    ...
    controller('HDBHTChargesCtrl', function ($scope, HDBHTChargesService, $http) {
        var t2 = 0;
        ...
        $scope.EditHTCharges = function (HTChargesId) {
            ...
            $('#DivBody').load('/Master/HDBMaster/EditHTCharges?HTChargesId=' + HTChargesId);
        ...
        $scope.ViewHTCharges = function (HTChargesId) {
            debugger;
            //var HTChargesId = $('#HTChargesId').val();
            $('#DivBody').load('/Master/HDBMaster/ViewHTCharges?HTChargesId=' + HTChargesId);
        ...
        function GetSlab() {
            ...
            HDBHTChargesService.GetSlab($scope.Size, $scope.ChargesFor).then(function (response) {
                debugger;
                ...
                if (HTChargesId > 0) {
                    HDBHTChargesService.EditHTChargesOther(HTChargesId).then(function (res) {
                        debugger;
                    ...
                    HDBHTChargesService.GetSlabChargeDtl(HTChargesId).then(function (response) {
                        debugger;
                        $scope.ChargeList = response.data;
                        //alert($scope.ChargeList);
                        //$scope.BtnHTSave = false;
                        //$scope.BtnHTList = true;
                    });
                }
            });
        }
        ...
        $scope.OnSizeChange = function () {
            ...
            HDBHTChargesService.GetSlab($scope.Size, $scope.ChargesFor).then(function (response) {
                ...
                $scope.WeightSlabList = response.data.LstWeightSlab;
                $scope.DistanceSlabList = response.data.LstDistanceSlab;
                $scope.CBMSlabList = response.data.LstCBMSlab;
        ...
        HDBHTChargesService.GetAllHTCharges().then(function (response) {
            debugger;
            // console.log(response.data);
        ...
        $scope.CBMSlabSelection = function (position, entities) {
            debugger;
            angular.forEach(entities, function (i, index) {
                debugger;
                if (position != index)
        ...
        $scope.AddHTCharges = function () {
            ...
            if ($scope.Slab == 'WithSlab') {
                ...
                var Cbm = 0;
                for (i = 0; i < $scope.CBMSlabList.length; i++) {
                    if ($scope.CBMSlabList[i].chkCBMSlab == true && $scope.CBMSlab == true) {
                        Cbm = 1;
                    }
                }
                if (Cbm == 0 && $scope.CBMSlab == true) {
                    alert('Please Select Atleast One Check Box From CBM Slab');
                    return false
                }
                debugger;
                //**************************************************************
                ...
                if ($scope.CBMSlab == true) {
                    var CbmSlabId = $.grep($scope.CBMSlabList, function (item) {
                        return item.chkCBMSlab == true;
                    })[0].CbmSlabId;
                    var FromCBMSlab = $.grep($scope.CBMSlabList, function (item) {
                        return item.chkCBMSlab == true;
                    })[0].FromCBMSlab;
                    var ToCBMSlab = $.grep($scope.CBMSlabList, function (item) {
                        return item.chkCBMSlab == true;
                    })[0].ToCBMSlab;
                    var chkCBMSlab = $.grep($scope.CBMSlabList, function (item) {
                        return item.chkCBMSlab == true;
                    })[0].chkCBMSlab;
                }
                ...
                if ($scope.WeightSlab == true && $scope.DistanceSlab == true) {
                    $scope.ChargeList.push({
                        ...
                        'AddCBMCharges': $('#AddCBMCharges').val()
                        //,
                        //'PortFromDistance': ($('#PortId').val() == null || $('#PortId').val() == "") ? 0 : 1,
                        //'PortToDistance': $('#PortToDistance').val()
                    });
                }
                else if ($scope.WeightSlab == true && $scope.DistanceSlab == true && $scope.CBMSlab == true) {
                    $scope.ChargeList.push({
                        'ID': t2,
                        'WtSlabId': WeightSlabId,
                        'FromWtSlabCharge': FromWeightSlab,
                        ...
                        'CBMSlabId': CbmSlabId,
                        'FromCBMSlabCharge': FromCBMSlab,
                        'ToCBMSlabCharge': ToCBMSlab,
        ...
        $scope.SaveHTCharges = function () {
            ...
            if ($scope.ChargeList.length != 0) {
                Obj = {
                    ...
                    IsReefer: $scope.IsReefer == true ? 1 : 0,
                }

                HDBHTChargesService.SaveHTCharges(Obj, $scope.ChargeList).then(function (res) {
                    debugger;
