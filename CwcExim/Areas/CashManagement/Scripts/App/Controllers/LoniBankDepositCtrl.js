(function () {
    ...
    .controller('PpgBankDepositCtrl', function ($scope, PpgBankDepositSvc) {
        ...
        $scope.GetBankDepositList = function () {
            $scope.Details = [];
            PpgBankDepositSvc.GetBankDepositList().then(function (r) {
                if (r.data.Status == 1) {
        ...
        $scope.AddEditBankDeposit = function () {
            ...
            if (($scope.Cash == 0 || $scope.Cash == null) && ($scope.Cheque == 0 || $scope.Cheque == null) && ($scope.NEFT == 0 || $scope.NEFT == null) && $scope.TemporaryAdvance.length == 0) {
                ...
                if (confirm('Are you sure to Save? Y/N')) {
                    ...
                    PpgBankDepositSvc.AddEditBankDeposit(obj).then(function (r) {
        ...
        $scope.DeleteBankDeposit = function (Id) {
            if (confirm('Are you sure to Delete? Y/N')) {
                PpgBankDepositSvc.DeleteBankDeposit(Id).then(function (r) {
                    if (r.data.Status == 1) {
                        $scope.GetBankDepositList();
                        alert(r.data.Message);
                        $scope.clear();
        ...
        $scope.GetNEFTForBankDeposit = function () {
            $scope.NEFT = 0;
            PpgBankDepositSvc.GetNEFTForBankDeposit($scope.DepositDate).then(function (r) {
                console.log(r.data);
                if (r.data.Status == 1) {
        ...
        $scope.Reset = function () {
            $('#DivBody').load('/CashManagement/Loni_CashManagement/BankDeposit');
        }
        ...
        //   PpgBankDepositSvc.GetExpenseHeadWithBalance().then(function (r) {
        //        console.log(r.data);
        //        if (r.data.Status == 1) {
        //            debugger;
        ...
        $scope.GetReceiptVoucherWithAmount = function (Head, EHId) {
            ...
            if (EHId != undefined) {
                ...
                PpgBankDepositSvc.GetReceiptVoucherBalance(EHId, DSNo).then(function (r) {
                    console.log(r.data);
                    if (r.data.Status == 1) {
                        debugger;
        ...
        $scope.GetAllDataForEdit = function (id) {
            ...
            PpgBankDepositSvc.GetBankDetailsIdByID(id).then(function (r) {
                console.log(r.data);
                if (r.data.Status == 1) {
                    debugger;
