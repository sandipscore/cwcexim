(function () {
    ...
    controller('ContMoveInvoiceEditCtrl', function ($scope, ContMoveInvoiceEditService) {
        $scope.InvoiceNo = "";
        $scope.conatiners = [];
        $scope.Message = '';
        ...
        $scope.SelectInvoice = function (obj) {
            ...
            ContMoveInvoiceEditService.GetYardInvoiceDetails($scope.InvoiceId).then(function (res) {
                ...
                $scope.PayeeName = $scope.InvoiceObj.ShippingLineName;
        ...
        $scope.ContainerSelect = function () {
            ...
            if (c > 0) {
                ContMoveInvoiceEditService.ContainerSelect($scope.InvoiceId, $('#InvoiceDate').val(), $scope.StuffingReqId, $scope.TaxType, $scope.conatiners).then(function (res) {
        ...
        $scope.GetInvoiceCharges = function () {
            ...
            var ChrgType = '';
            if ($scope.InvoiceObj.lstPostPaymentChrg.filter(x=>x.Clause == 'GRL').length > 0) {
                ChrgType='GR'
            }
            else if ($scope.InvoiceObj.lstPostPaymentChrg.filter(x=>x.Clause == 'RR').length > 0) {
                ChrgType = 'RR'
            }
            else if ($scope.InvoiceObj.lstPostPaymentChrg.filter(x=>x.Clause == 'THC').length > 0) {
                ChrgType = 'RR'
            }
            else if ($scope.InvoiceObj.lstPostPaymentChrg.filter(x=>x.Clause == 'FNC').length > 0) {
                ChrgType = 'FMC'
            }
            else if ($scope.InvoiceObj.lstPostPaymentChrg.filter(x=>x.Clause == 'WHT').length > 0) {
                ChrgType = 'W'
            }
            else {
                alert("Something Wrong In Charge.");
                return;
            }
            ContMoveInvoiceEditService.GetInvoiceCharges($scope.InvoiceObj.RequestId,
                $scope.InvoiceObj.lstPostPaymentCont[0].ContainerNo,
                $('#InvoiceDate').val(),
                'Tax', $scope.InvoiceObj.LocationId, $scope.PartyId ,
                ChrgType,
                0,
                $scope.InvoiceId
                ).then(function (res) {
        ...
        $scope.SubmitInvoice = function () {
           ...
           // }
            ...
            if (confirm('Are you sure to Update this Invoice?')) {
                ...
                ContMoveInvoiceEditService.GenerateInvoice($scope.InvoiceObj).then(function (res) {
