(function () {
    ...
    controller('WFLDReservationCtrl', function ($scope, WFLDReservationService) {
        var d = new Date();
        var n = d.getFullYear();
        ...
        var resobj1 = new WFLDReservationModel();
        ...
        $scope.CreateInvoice = function () {
            ...
            if ($scope.Month != null && $scope.Month != '' && $scope.Year != null && $scope.Year != '') {
                ...
                WFLDReservationService.CreateResInvoices($scope.Month, $scope.Year, $scope.PartyId, 1).then(function (res) {
                    //debugger;
        ...
        $scope.Save = function () {
            ...
            if (confirm('Are you sure to save?')) {
                ...
                debugger;
                WFLDReservationService.SaveGenerateInvoice($scope.InvDtls, $scope.Month, $scope.Year).then(function (res) {
                    ...
                    if (res.data.Status == 1) {
                        ...
                        for (i = 0; i < $scope.PartyInv.length; i++) {
                            for (j = 0; j < $scope.InvDtls.length ; j++) {
                                if ($scope.InvDtls[j].PartyId == $scope.PartyInv[i].PartyId && $scope.InvDtls[j].GodownId == $scope.PartyInv[i].GodownId) {
        ...
        $scope.ChangePartyList = function () {
            ...
            if (Month != '' && Year > 0) {
                WFLDReservationService.GetReservationParties($scope.Month, $scope.Year).then(function (res) {
                    console.log(res.data);
