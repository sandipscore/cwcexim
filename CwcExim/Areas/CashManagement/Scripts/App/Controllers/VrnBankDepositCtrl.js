(function () {
    ...
    .controller('VrnBankDepositCtrl', function ($scope, VrnBankDepositSvc) {
        ...
        $scope.GetBankDepositList = function () {
            $scope.Details = [];
            VrnBankDepositSvc.GetBankDepositList().then(function (r) {
                if (r.data.Status == 1) {
        ...
        $scope.AddEditBankDeposit = function () {
            debugger;
            if (($scope.Cash == 0 || $scope.Cash == null) && ($scope.Cheque == 0 || $scope.Cheque == null) && ($scope.NEFT == 0 || $scope.NEFT == null) ) {
                alert('Please Input Bank Deposit Value');
            }
            else {
                ...
                if (confirm('Are you sure to Save? Y/N')) {
                    ...
                    VrnBankDepositSvc.AddEditBankDeposit(obj).then(function (r) {
        ...
        $scope.DeleteBankDeposit = function (Id) {
            if (confirm('Are you sure to Delete? Y/N')) {
                VrnBankDepositSvc.DeleteBankDeposit(Id).then(function (r) {
                    if (r.data.Status == 1) {
                        $scope.GetBankDepositList();
                        alert(r.data.Message);
                        $scope.clear();
        ...
        $scope.GetNEFTForBankDeposit = function () {
            $scope.NEFT = 0;
            VrnBankDepositSvc.GetNEFTForBankDeposit($scope.DepositDate).then(function (r) {
                console.log(r.data);
                if (r.data.Status == 1) {
        ...
        $scope.Reset = function () {
            $('#DivBody').load('/CashManagement/VRN_CashManagement/BankDeposit');
        }
        ...
        $scope.GetReceiptVoucherWithAmount = function (Head, EHId) {
            ...
            if (EHId != undefined) {
                ...
                /*
                VrnBankDepositSvc.GetReceiptVoucherBalance(EHId, DSNo).then(function (r) {
                    console.log(r.data);
                    if (r.data.Status == 1) {
                        debugger;
        ...
        $scope.GetAllDataForEdit = function (id) {
            ...
            VrnBankDepositSvc.GetBankDetailsIdByID(id).then(function (r) {
                console.log(r.data);
                if (r.data.Status == 1) {
                    debugger;
