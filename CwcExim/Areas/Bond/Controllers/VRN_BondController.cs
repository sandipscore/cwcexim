...
{
    public class VRN_BondController : Controller
    {
        ...
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            VRN_BondApp objBA = new VRN_BondApp();
        ...
        public JsonResult AddEditBondApp(VRN_BondApp objBA)
        {
        ...
        public ActionResult EditBondApp(int SpaceappId)
        {
            ...
            VRN_BondRepository objHdb = new VRN_BondRepository();
            VRN_BondApp objBA = new VRN_BondApp();
            ViewData["cha"] = ((Login)Session["LoginUser"]).CHA;
            if (Convert.ToBoolean(ViewData["cha"]) == false)
            {
            ...
            if (objHdb.DBResponse.Data != null)
                objBA = (VRN_BondApp)objHdb.DBResponse.Data;
            return PartialView(objBA);
        }
        [HttpGet]
        public ActionResult ViewBondApp(int SpaceappId)
        {
            ...
            if (objBR.DBResponse.Data != null)
                objBA = (VRN_BondApp)objBR.DBResponse.Data;
            return PartialView(objBA);
        }
        [HttpGet]
        ...
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            IList<VRN_ListOfBondApp> lstApp = new List<VRN_ListOfBondApp>();
            objBR.ListOfSpaceAvailability();
            if (objBR.DBResponse.Data != null)
                lstApp = (List<VRN_ListOfBondApp>)objBR.DBResponse.Data;
        ...
        public JsonResult PrintBondApp(int SpaceappId)
        {
            ...
            if (objBR.DBResponse.Data != null)
            {
                VRN_SpaceAvailAppCertPdf ObjSpaceAvail = new VRN_SpaceAvailAppCertPdf();
                ObjSpaceAvail = (VRN_SpaceAvailAppCertPdf)objBR.DBResponse.Data;
        ...
        [NonAction]
        public string GeneratePDFForSpaceAvailAppCert(VRN_SpaceAvailAppCertPdf ObjSpaceAvail, int SpaceappId)
        {
            string Html = "", Htmlnew;
        ...
        {
            VRN_BondRepository ObjBR = new VRN_BondRepository();
            VRN_SpaceAvailableCert ObjSpaceAvail = new VRN_SpaceAvailableCert();
            ObjBR.GetSpaceAvailCertById(SpaceappId, Status);
            if (ObjBR.DBResponse.Data != null)
            {
                ObjSpaceAvail = (VRN_SpaceAvailableCert)ObjBR.DBResponse.Data;
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddEditSpaceAvailCert(VRN_SpaceAvailableCert ObjSpaceAvail)
        {
        ...
        public ActionResult PrintSpaceAvailCertApprove(int SpaceappId)
        {
            ...
            if (ObjBR.DBResponse.Data != null)
            {
                VRN_SpaceAvailCertPdf ObjSpaceAvail = new VRN_SpaceAvailCertPdf();
                ObjSpaceAvail = (VRN_SpaceAvailCertPdf)ObjBR.DBResponse.Data;
        ...
        [NonAction]
        public string GeneratePDFForSpaceAvailCertForApprove(VRN_SpaceAvailCertPdf ObjSpaceAvail, int SpaceappId)
        {
            string Html = "", Htmlnew;
        ...
        [NonAction]
        public string GeneratePDFForSpaceAvailCertForReject(VRN_SpaceAvailCertPdf ObjSpaceAvail, int SpaceappId)
        {
            string Html = "", Htmlnew;
            string Path = Server.MapPath("~/Docs/") + Session.SessionID + "/SpaceAvailCert" + SpaceappId + ".pdf";
            if (!Directory.Exists(Server.MapPath("~/Docs/") + Session.SessionID))
        ...
        {
            VRN_BondRepository obj = new VRN_BondRepository();
        ...
        public ActionResult CreateBondAdvPaymentSheet(string type = "Tax")
        {
            ...
            //VRN_BondRepository objBond = new VRN_BondRepository();
        ...
        [HttpGet]
        public JsonResult GetBondAdvPaymentSheet(string InvoiceDate, string ExportUnder, string InvoiceType, int StuffingReqId, int DeliveryType, string StuffingReqNo, string StuffingReqDate, string UptoDate, decimal Area,
           int PartyId, string PartyName, string PartyAddress, string PartyState, string PartyStateCode, string PartyGST, int PayeeId, string PayeeName, int CasualLabour = 0, int NoOfPrinting = 0, int OTHour = 0
           )
        {

            VRN_BondRepository objBond = new VRN_BondRepository();
            objBond.GetBondAdvancePaymentSheet(InvoiceDate, ExportUnder, StuffingReqId, DeliveryType, StuffingReqNo, UptoDate, PartyId, PartyName, PartyAddress, PartyState,
                    PartyStateCode, PartyGST, PayeeId, PayeeName, CasualLabour, InvoiceType, Area, 0, NoOfPrinting, OTHour);

            var objBondPostPaymentSheet = new VRN_PostPaymentSheet();
            var Output = (VRN_PostPaymentSheet)objBond.DBResponse.Data;
        ...
        public JsonResult AddEditBondAdvPaymentSheet(FormCollection objForm)
        {
            try
            {
                ...
                var invoiceData = JsonConvert.DeserializeObject<VRN_PostPaymentSheet>(objForm["PaymentSheetModelJson"].ToString());
                string ContainerXML = "";
                string ChargesXML = "";
                ...
                VRN_BondRepository objChargeMaster = new VRN_BondRepository();
        ...
        {
            VRN_BondRepository objER = new VRN_BondRepository();
            objER.ListOfBondInvoice("BNDAdv", InvoiceNo, InvoiceDate);
            List<VRN_ListOfBondInvoice> obj = new List<VRN_ListOfBondInvoice>();
            if (objER.DBResponse.Data != null)
                obj = (List<VRN_ListOfBondInvoice>)objER.DBResponse.Data;
            return PartialView("ListOfInvoiceBondAdv", obj);
        }

        [HttpGet]
        public JsonResult GetSacNum()
        {
            VRN_BondRepository objBond = new VRN_BondRepository();
            objBond.GetSACNoForAdvBondPaymentSheet();
        ...
        {
            VRN_DepositApp ObjDepositApp = new VRN_DepositApp();
            ...
            VRN_BondRepository ObjWlB = new VRN_BondRepository();
            ObjWlB.GetSacNoForDepositApp();
            if (ObjWlB.DBResponse.Data != null)
            {
                ViewBag.SacNoList = JsonConvert.DeserializeObject(JsonConvert.SerializeObject(ObjWlB.DBResponse.Data));
            ...
            ObjWlB.ListOfImporter();
            if (ObjWlB.DBResponse.Data != null)
                ViewBag.ListOfImporter = ObjWlB.DBResponse.Data;
            else ViewBag.ListOfImporter = null;

            ObjWlB.ListOfGodown();
            if (ObjWlB.DBResponse.Data != null)
                ViewBag.GodownList = new SelectList((List<Import.Models.Godown>)ObjWlB.DBResponse.Data, "GodownId", "GodownName");
            else
        ...
        {
            VRN_BondRepository ObjCR = new VRN_BondRepository();
        ...
        public ActionResult ViewDepositApp(int DepositAppId)
        {
            ...
            {
                VRN_BondRepository ObjWlB = new VRN_BondRepository();
                ObjWlB.GetDepositApp(DepositAppId);
                if (ObjWlB.DBResponse.Data != null)
                {
                    ObjDepositApp = (VRN_DepositApp)ObjWlB.DBResponse.Data;
                }
            }
        ...
        public ActionResult GetDepositAppList()
        {
            BondRepository ObjBR = new BondRepository();
            VRN_BondRepository ObjWlB = new VRN_BondRepository();
            ObjWlB.GetAllDepositApp();
            List<VRN_DepositApp> LstDepositApp = new List<VRN_DepositApp>();
            if (ObjWlB.DBResponse.Data != null)
            {
                LstDepositApp = (List<VRN_DepositApp>)ObjWlB.DBResponse.Data;
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddEditDepositApp(VRN_DepositApp ObjDepositApp)
        {
            if (ModelState.IsValid)
            {
        ...
        public JsonResult PrintDepositeApp(int DepositAppId)
        {
            ...
            {
                VRN_BondRepository ObjWlb = new VRN_BondRepository();
                VRN_PrintDA objDA = new VRN_PrintDA();
                ObjWlb.PrintDA(DepositAppId);
                if (ObjWlb.DBResponse.Data != null)
                {
                    objDA = (VRN_PrintDA)ObjWlb.DBResponse.Data;
        ...
        [HttpPost]
        [CustomValidateAntiForgeryToken]
        public JsonResult DeleteBondDepositApp(int DepositAppId)
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            objBR.DeleteBondDepositApp(DepositAppId);
            return Json(objBR.DBResponse);
        }
        #endregion
        ...
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            VRN_BondWOUnloading objBond = new VRN_BondWOUnloading();
            ...
            if (objBR.DBResponse.Data != null)
            {
                ...
                objBond = JsonConvert.DeserializeObject<VRN_BondWOUnloading>(JsonConvert.SerializeObject(jobject["objBond"]));
            }
        ...
        public ActionResult ViewWOUnloading(int UnloadingId)
        {
            ...
            if (objBR.DBResponse.Data != null)
                objBond = (VRN_BondWOUnloading)objBR.DBResponse.Data;
            return PartialView(objBond);
        }
        [HttpGet]
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult AddEditWOUnloading(VRN_BondWOUnloading obj)
        {
            if (ModelState.IsValid)
            {
                VRN_BondRepository objBR = new VRN_BondRepository();
        ...
        public JsonResult PrintWOUnloading(int UnloadingId)
        {
            ...
            if (objBR.DBResponse.Data != null)
            {
                VRN_BondWOUnloadingPDF ObjUnload = new VRN_BondWOUnloadingPDF();
                ObjUnload = (VRN_BondWOUnloadingPDF)objBR.DBResponse.Data;
        ...
        [NonAction]
        public string GeneratePDFForWOUnloading(VRN_BondWOUnloadingPDF ObjUnload, int UnloadingId)
        {
            string Html = "";
        ...
        [HttpPost]
        [CustomValidateAntiForgeryToken]
        public JsonResult DeleteBondUnloadingApp(int UnloadingId)
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            objBR.DeleteBondUnloadingApp(UnloadingId);
            return Json(objBR.DBResponse);
        }
        #endregion
        ...
        {
            VRN_BondRepository ObjBR = new VRN_BondRepository();
            VRN_DeliveryOrder ObjDeliveryOrder = new VRN_DeliveryOrder();
            ObjBR.GetDeliveryOrder(DeliveryOrderId);
            if (ObjBR.DBResponse.Data != null)
            {
                ObjDeliveryOrder = (VRN_DeliveryOrder)ObjBR.DBResponse.Data;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult AddEditDeliveryOrder(VRN_DeliveryOrder ObjDeliveryOrder)
        {
            if (ModelState.IsValid)
            {
                VRN_BondRepository ObjBR = new VRN_BondRepository();
                ...
                {
                    ObjDeliveryOrder.LstDeliveryOrderhdb = Newtonsoft.Json.JsonConvert.DeserializeObject<List<VRN_DeliveryOrderDtl>>(ObjDeliveryOrder.DeliveryOrderXml);
        ...
        public JsonResult PrintDeliveryApp(int DeliveryAppId)
        {
            ...
            {
                VRN_BondRepository ObjHdb = new VRN_BondRepository();
                VRN_DeliveryOrder objDA = new VRN_DeliveryOrder();
                ObjHdb.PrintDelivery(DeliveryAppId);
                if (ObjHdb.DBResponse.Data != null)
                {
                    objDA = (VRN_DeliveryOrder)ObjHdb.DBResponse.Data;
                    ...
                    html += "<h1 style='font-size: 22px; margin: 0; padding: 0;'>CENTRAL WAREHOUSING CORPORATION</h1><span style='font-size:12px;'>(A Govt. of India Undertaking)</span><br/><span style='font-size:12px;'>" + objDA.CompanyAddress + "</span><br/><label style='font-size: 14px;'>" + objDA.CompanyEmail + "</label><br/><label style='font-size: 14px; font-weight:bold;'>DELIVERY ORDER (CUSTOM BOND)</label><br/><label style='font-size: 13px;'>(To be filled by Importer / Agent)</label></td>";
                    ...
                    html += "</tbody></table></td><td colspan='6' width='50%'><table cellspacing='0' cellpadding='0' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 5px; '><tbody><tr><th style='font-size:13px; text-align:right;'>Central Warehouse : <span>&nbsp;</span></th><td style='font-size:12px; width:27%;'>" + objDA.CompanyLoaction + "</td></tr></tbody></table></td></tr><tr>";
                    ...
                    html += "<tr><td colspan='6' width='50%'><table cellspacing='0' cellpadding='0' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 5px; '><tbody><tr><td colspan='8' style='font-size:12px;'>As per the Ex-Bond Bill of Entry No. <u>" + objDA.LstDeliveryOrderhdb[0].ExBoeNo + "</u></td></tr></tbody></table></td><td colspan='6' width='50%'><table cellspacing='0' cellpadding='5' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 5px; '><tbody><tr><th style='font-size:13px; text-align:right;'>Dated</th> <td style='font-size:12px; width:20%;'><u>" + objDA.LstDeliveryOrderhdb[0].ExBoeDate + "</u></td></tr></tbody></table></td></tr></tbody></table></td></tr>";
                    ...
                    html += "<tr><td width='50%'></td> <td colspan='6' width='50%'><table cellspacing='0' cellpadding='5' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0;'><tbody><tr><td colspan='6' width='60%' align='right' style='font-size:12px;'>SGST Amount :</td><td colspan='6' width='40%' align='left'><p style='margin:0; font-size:12px;'>Rs. <u>" + SGST + " </u> </p></td></tr></tbody></table></td></tr>";
                    html += "<tr><td width='50%'></td> <td colspan='6' width='50%'><table cellspacing='0' cellpadding='5' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0;'><tbody><tr><td colspan='6' width='60%' align='right' style='font-size:12px;'>CGST Amount :</td><td colspan='6' width='40%' align='left'><p style='margin:0; font-size:12px;'>Rs. <u>" + CGST + "</u> </p></td></tr></tbody></table></td></tr>";
                    html += "<tr><td width='50%'></td> <td colspan='6' width='50%'><table cellspacing='0' cellpadding='5' style='width:100%; font-family: Arial, Helvetica, sans-serif; margin: 0;'><tbody><tr><td colspan='6' width='60%' align='right' style='font-size:12px;'>IGST Amount :</td><td colspan='6' width='40%' align='left'><p style='margin:0; font-size:12px;'>Rs. <u>" + IGST + "</u> </p></td></tr></tbody></table></td></tr>";
        ...
        {
            VRN_SpaceAvailCertExtend ObjSpaceAvail = new VRN_SpaceAvailCertExtend();
            ObjSpaceAvail.ExtendOn = DateTime.Now.ToString("dd/MM/yyyy");
            VRN_BondRepository ObjBR = new VRN_BondRepository();
            ObjBR.GetSacNo();
            if (ObjBR.DBResponse.Data != null)
            {
                ViewBag.SacNoList = new SelectList((List<VRN_SpaceAvailableCert>)ObjBR.DBResponse.Data, "SpaceappId", "SacNo");
            }
            else
            {
                ViewBag.SacNoList = null;
        ...
        public ActionResult EditSpaceAvailCertExtend(int SpaceAvailCertExtId)
        {
            ...
            if (SpaceAvailCertExtId > 0)
            {
                ...
                if (ObjBR.DBResponse.Data != null)
                {
                    ObjSpaceAvail = (VRN_SpaceAvailCertExtend)ObjBR.DBResponse.Data;
                }
            }
        ...
        public ActionResult GetSpaceAvailCertExtendList()
        {
            ...
            List<VRN_SpaceAvailCertExtend> LstSpaceAvail = new List<VRN_SpaceAvailCertExtend>();
            if (ObjBR.DBResponse.Data != null)
            {
                LstSpaceAvail = (List<VRN_SpaceAvailCertExtend>)ObjBR.DBResponse.Data;
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddEditSpaceAvailCertExtend(VRN_SpaceAvailCertExtend ObjSpaceAvail)
        {
        ...
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            VRN_BondAppExtendDetails objBD = new VRN_BondAppExtendDetails();
            objBR.GetSpaceAvailabilityExt(SpaceAvailCertExtId);
            if (objBR.DBResponse.Data != null)
                objBD = (VRN_BondAppExtendDetails)objBR.DBResponse.Data;
        ...
        {
            VRN_BondRepository objBR = new VRN_BondRepository();
            VRN_SpaceAvailCertPdf objSAC = new VRN_SpaceAvailCertPdf();
            objBR.PrintSACExt(SpaceAvailCertExtId, Convert.ToInt32(Session["BranchId"]));
            if (objBR.DBResponse.Data != null)
            {
                objSAC = (VRN_SpaceAvailCertPdf)objBR.DBResponse.Data;
        ...
        [NonAction]
        public string GenerateExtSAC(VRN_SpaceAvailCertPdf objSAC, int SpaceAvailCertExtId)
        {
            string Html = "";
        ...
        public ActionResult CreateBondAdvPaymentSheetInterim(string type = "Tax")
        {
            ...
            if (objBond.DBResponse.Status > 0)
                ...
                ViewBag.SACList = null;
            VRN_BondRepository objWBond = new VRN_BondRepository();
        ...
        {
            VRN_BondRepository objExport = new VRN_BondRepository();
        ...
        [HttpGet]
        public JsonResult GetPayerName()
        {
            VRN_BondRepository objExport = new VRN_BondRepository();
        ...
        {
            VRN_BondRepository objExport = new VRN_BondRepository();
            objExport.GetPaymentPartyForAdvanceBond("", 0);
            if (objExport.DBResponse.Data != null)
            {
                return Json(objExport.DBResponse.Data, JsonRequestBehavior.AllowGet);
            }
        ...
        [HttpGet]
        public JsonResult GetBondAdvPaymentSheetInterim(string InvoiceDate, string InvoiceType, int StuffingReqId, int DeliveryType, string StuffingReqNo, string StuffingReqDate, string UptoDate, decimal Area,
            int PartyId, string PartyName, string PartyAddress, string PartyState, string PartyStateCode, string PartyGST, int PayeeId, string PayeeName, int CasualLabour)
        {
            ...
            VRN_BondRepository objBond = new VRN_BondRepository();
            objBond.GetBondAdvancePaymentSheetInterim(InvoiceDate, StuffingReqId, DeliveryType, StuffingReqNo, UptoDate, PartyId, PartyName, PartyAddress, PartyState,
        ...
        public ActionResult BondDeliPaymentSheet(string type = "Tax")
        {
            ...
            //if (objBond.DBResponse.Status > 0)
            //{
            //    var BondList = (IList<VRN_BondSacDetails>)objBond.DBResponse.Data;
            //    TempData["SACList"] = BondList;

            //    IList<VRN_BondSacDetails> DistinctSacList = new List<VRN_BondSacDetails>();
        ...
        public JsonResult GetBondDelPaymentSheet(string InvoiceDate, string InvoiceType, int StuffingReqId, int DeliveryType, string StuffingReqNo, string StuffingReqDate, string UptoDate, decimal Area,
            ...
            int Units, int IsInsured, string DepositDate, string BOENo, string BOEDate, string ImporterExporter, string CHAName, string ExportUnder, int InvoiceId = 0,int OTHour=0)
        {
            ...
            objBond.GetBondDelPaymentSheet(InvoiceDate, InvoiceType, StuffingReqId, DeliveryType, StuffingReqNo, StuffingReqDate, UptoDate, Area,
             ...
             Units, IsInsured, DepositDate, BOENo, BOEDate, ImporterExporter, CHAName, ExportUnder, InvoiceId, OTHour);


            var objBondPostPaymentSheet = (VRN_BondPostPaymentSheet)objBond.DBResponse.Data;
            var objBondList = ((IList<VRN_BondSacDetails>)TempData.Peek("SACList")).Where(o => o.DepositAppId == StuffingReqId).ToList();
        ...
        public JsonResult AddEditBondDelPaymentSheet(FormCollection objForm)
        {
            try
            {
                ...
                var invoiceData = JsonConvert.DeserializeObject<VRN_BondPostPaymentSheet>(objForm["PaymentSheetModelJson"].ToString());
                string ChargesXML = "";
