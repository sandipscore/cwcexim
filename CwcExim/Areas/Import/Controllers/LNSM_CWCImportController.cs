...
{
    //For SRS Version 3.2 
    public class LNSM_CWCImportController : BaseController
    {
        ...
        {
            Loni_ImportRepositoryV2 ObjRR = new Loni_ImportRepositoryV2();
        ...
        {
            Loni_ImportRepositoryV2 objGPR = new Loni_ImportRepositoryV2();
        ...
        public JsonResult AddEditInternalMovementApp(PPG_Internal_MovementAppV2 objForm)
        {
            try
            {
                ...
                Loni_ImportRepositoryV2 objChargeMaster = new Loni_ImportRepositoryV2();
        ...
        #region Train Summary Upload LONI SAMO

        [HttpGet]
        public ActionResult TrainSummaryUploadLONISAMO()
        {            
            return PartialView();
        }

        [HttpPost]
        public JsonResult CheckUploadLONISAMO(string TrainNo, string TrainDate)
        {
            int status = 0;
            string TrainNumber = "";
            string TrainDt = "";
            ...
            List<LNSM_TrainSummaryUpload> TrainSummaryUploadList = new List<LNSM_TrainSummaryUpload>();
            if (Request.Files.Count > 0)
            {
                HttpPostedFileBase File = Request.Files[0];
                string extension = Path.GetExtension(File.FileName);

                log.Info("Method Start: CheckUploadLONISAMO");
                        ...
                        if (dt.Rows.Count > 8)
                        {
                            try
                            {
                                for (int i = 1; i <= 5; i++)
                                {
                                    if (i == 1)
                                    {
                                        TrainNumber = dt.Rows[1][1].ToString();
                                    }
                                    if (i == 2)
                                    {
                                        TrainDt = dt.Rows[2][1].ToString();
                                    }
                                    if (i == 3)
                                    {
                                        Startinglocation = dt.Rows[3][1].ToString();
                                    }
                                    if (i == 4)
                                    {
                                        Dest = dt.Rows[4][1].ToString();
                                    }
                                    if (i == 5 )
                                    {
                                        Remark = dt.Rows[5][1].ToString();
                                    }
                                }
                                    ...
                                    for (int j = 9; j < dt.Rows.Count; j++)
                                {
                                        ...
                                        LNSM_TrainSummaryUpload objTrainSummaryUpload = new LNSM_TrainSummaryUpload();
                                        objTrainSummaryUpload.TrainNo = TrainNumber;
                                        objTrainSummaryUpload.TrainDate = TrainDt;
                                        objTrainSummaryUpload.StartLocation = Startinglocation;
                                        objTrainSummaryUpload.Destination = Dest;
                                        objTrainSummaryUpload.Remarks = Remark;

                                        objTrainSummaryUpload.NormalHub = dt.Rows[j][1].ToString();
                                        objTrainSummaryUpload.Wagon_No = dt.Rows[j][2].ToString();
                                        objTrainSummaryUpload.Stack = dt.Rows[j][3].ToString();
                                        objTrainSummaryUpload.Container_No = dt.Rows[j][4].ToString();
                                        objTrainSummaryUpload.LE = dt.Rows[j][5].ToString();
                                        objTrainSummaryUpload.CT_Size = dt.Rows[j][6].ToString();
                                        objTrainSummaryUpload.Cargo = dt.Rows[j][7].ToString();
                                        objTrainSummaryUpload.Cargo_Wt = Convert.ToDecimal(dt.Rows[j][8].ToString());
                                        objTrainSummaryUpload.Ct_Tare = Convert.ToDecimal(dt.Rows[j][9].ToString());
                                        objTrainSummaryUpload.Total_Wt = Convert.ToDecimal(dt.Rows[j][10].ToString());
                                        objTrainSummaryUpload.Exim = dt.Rows[j][11].ToString(); 
                                        TrainSummaryUploadList.Add(objTrainSummaryUpload);
                                ...
                                LNSM_ImportRepository objImport = new LNSM_ImportRepository();
                                objImport.CheckTrainSummaryUploadLONISAMO(TrainNo, TrainSummaryUploadXML, TrainDate);
                                if (objImport.DBResponse.Status > -1)
                                {
                                    status = Convert.ToInt32(objImport.DBResponse.Status);
                                    TrainSummaryUploadList = ((List<LNSM_TrainSummaryUpload>)objImport.DBResponse.Data).Where(x => x.Status == 0).ToList();
                                }
                                else
                                {
                                    status = -6;
        ...
        public DataTable ConvertCsvToDataTable(string filePath)
        {
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                // Create columns in DataTable based on CSV headers
        ...
        [HttpPost]
        public ActionResult SaveUploadDataLONISAMO(List<LNSM_TrainSummaryUpload> objTrainSummaryUpload)
        {
            LNSM_ImportRepository objImport = new LNSM_ImportRepository();

            string TrainSummaryUploadXML = Utility.CreateXML(objTrainSummaryUpload);

            objImport.AddUpdateTrainSummaryUploadLONISAMO(TrainSummaryUploadXML);
            return Json(objImport.DBResponse);
        }

        [HttpGet]
        public ActionResult ListOfTrainSummaryLONISAMO()
        {
            LNSM_ImportRepository objER = new LNSM_ImportRepository();
            List<LNSM_TrainSummaryUpload> lstCargoSeize = new List<LNSM_TrainSummaryUpload>();
            objER.ListOfTrainSummaryLONISAMO();
            if (objER.DBResponse.Data != null)
                lstCargoSeize = (List<LNSM_TrainSummaryUpload>)objER.DBResponse.Data;
            return PartialView(lstCargoSeize);
        }

        [HttpGet]
        public ActionResult GetTrainSummaryDetailsLONISAMO(int TrainSummaryUploadId)
        {
            LNSM_ImportRepository objImport = new LNSM_ImportRepository();
            objImport.GetTrainSummaryDetailsLONISAMO(TrainSummaryUploadId);
            return Json(objImport.DBResponse.Data, JsonRequestBehavior.AllowGet);
        }
