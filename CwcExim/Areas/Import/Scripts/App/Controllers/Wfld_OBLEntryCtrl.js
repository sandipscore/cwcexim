(function () {
    angular.module('CWCApp').directive('autofocus', ['$timeout', function ($timeout) {
        return {
            restrict: 'A',
    ...
    angular.module('CWCApp').factory('focus', function ($rootScope, $timeout) {
    ...
    angular.module('CWCApp').controller('Wfld_OBLEntryCtrl', function ($scope, Wfld_OBLEntryService, focus) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        focus('focusMe');
      ...
      $scope.OblEntryDetails.length = 0;
        ...
        var inOriPort = 0;
        $scope.onPortCodeOriginChange = function (index) {
            debugger;
            inOriPort = index;
        }
              
        $scope.SelectPortCodeOrigin = function (PortId, PortName) {
            debugger;
            //$scope.OblEntryDetails[inOriPort].PortId = PortId;
            $scope.OblEntryDetails[inOriPort].PortCodeOrigin = PortName;
            $('#PortCodeOriginBox').val('');
            $("#PortCodeOriginModal").modal("hide");
            LoadPortCodeOrigin();
            $scope.$applyAsync();
            $("#PortCodeOriginModal").modal("hide");
            ...
            inImp = index;
              ...
              $scope.OBLSelectShippingLine = function (ShippingLineId, ShippingLineName) {
                  debugger;
        ...
        $scope.AddOblEntry = function () { 
            ...
            if (len > 0) {
                ...
                if ($scope.OblEntryDetails[len - 1].ArrivalDate == '' || $scope.OblEntryDetails[len - 1].ArrivalDate == null) {
                    alert("Please fillup the row");
                    return false;
                }
                ...
                if ($scope.OblEntryDetails[len - 1].CIFValue == '' || $scope.OblEntryDetails[len - 1].CIFValue == null || $scope.OblEntryDetails[len - 1].CIFValue == 0) {
                    alert("Please fillup the row");
                    return false;
                }

                if ($scope.OblEntryDetails[len - 1].PortCodeOrigin == '' || $scope.OblEntryDetails[len - 1].PortCodeOrigin == null) {
                    alert("Please fillup the row");
                    return false;
                }
        ...
        $scope.GetOBLDetails = function () {
            ...
            var PortCodeOriginName = $('#PortCodeOriginName').val();
            //var GatewayPortCode = $('#GatewayPortCode').val();

            Wfld_OBLEntryService.GetOBLDetails(ContainerNo, CFSCode, ContainerSize, OBLEntryId).then(function (response) {
                debugger;
                $scope.OblEntryDetails = [];
                      ...
                      //if (j == 0) {
                          $scope.OblEntryDetails.push
                           ({
                                  ...
                                  'PortCodeOrigin': item.PortCodeOrigin,
                                  'GatewayPortCode': item.GatewayPortCode,
                    ...
                    debugger;
                        $scope.OblEntryDetails.push
                         ({
                             ...
                             'ContainerNo': item.ContainerNo,
                             'ContainerSize': item.ContainerSize,
                             'ArrivalDate': item.ArrivalDate == "" ? null : item.ArrivalDate,
                             'IGM_No': item.IGM_No,
                             'IGM_Date': item.IGM_Date == "" ? null : item.IGM_Date,
                             'LineNo': item.LineNo,
                             'TSANo': item.TSANo,
                             'TSA_Date': item.TSA_Date==""?null:item.TSA_Date,
                             'SMTPNo': item.SMTPNo,
                             'SMTP_Date': item.SMTP_Date==""?null:item.SMTP_Date,
                             'NoOfPkg': item.NoOfPkg,
                             'CargoType': item.CargoType == 0 ? "0" : item.CargoType == 1 ? "1" :  item.CargoType == 2 ? "2":"3",
                             ...
                             //'IGM_IMPORTER': item.IGM_IMPORTER,
                             'AreaCBM': item.AreaCBM == '' ? 0 : item.AreaCBM,
                             'CIFValue': item.CIFValue,
                             'PortCodeOrigin': item.PortCodeOrigin,
                             'GatewayPortCode': item.GatewayPortCode,
                         });
        ...
        $scope.OnOBLEntrySave = function () {
            ...
            //if ($('#GatewayPortCode').val() == '' || $('#PortId').val() == 0) {
            //    alert('Please Select Gateway Port Code.');
            //    return false;
            //}
            ...
            angular.forEach($scope.OblEntryDetails, function (item) {
                ...
                else if (item.SMTP_Date == '') {
                    flag8 = 1;
                }
                else if (item.PortCodeOrigin == '') {
                    flag9 = 1;
                }
            ...
            if (flag7 == 1) {
                alert('Please Select SMTP No.');
                return false;
            }

            if (flag8 == 1) {
                alert('Please Select SMTP Date');
                return false;
            }
            if (flag9 == 1) {
                alert('Please Select Port Code Origin');
                return false;
            }
            debugger;
            var len = $scope.OblEntryDetails.length;
            ...
            if (confirm('Are you sure to save OBL Entry?')) {
                ...
                Wfld_OBLEntryService.OBLEntrySave(Obj, JSON.stringify($scope.OblEntryDetails)).then(function (res) {
                    console.log(res);
        ...
        function LoadOblEntry() {
            $('#DivBody').load('/Import/Wfld_OblEntry/OBLEntry');
        }
        ...
        $scope.CallCommodityModal = function (impc) {
            inComo = impc;
            var CharCode;
            if (window.event) {
        ...
        $scope.CallPortCodeOriginModal = function (impo) {
            inOriPort = impo;
            var CharCode;
            if (window.event) {
                CharCode = window.event.keyCode;
            ...
            if (CharCode == 32) {
                debugger;                
                $("#PortCodeOriginModal").modal('show');
                $('#PortCodeOriginModal').on('shown.bs.modal', function () {
                    $('#PortCodeOriginBox').focus();
                })                
            }
        }


        $scope.CallImporterModal = function (impi) {
            inImp = impi;
            var CharCode;
            if (window.event) {
                CharCode = window.event.keyCode;
        ...
        $scope.CallShippingModal = function (imps) {
            inShipp = imps;
            var CharCode;
            if (window.event) {
                CharCode = window.event.keyCode;
            ...
            if (CharCode == 32) {
                ...
                $("#OBLShippingLineModal").modal('show');
                $('#OBLShippingLineModal').on('shown.bs.modal', function () {
                    $('#OBLShpngLinebox').focus();
                })
                //   var tabindex = 102;
                debugger;
                //$('#tbleSline  tr').each(function () {
        ...
        $scope.CallGatewayPortCodemodal = function (impg) {
            debugger;
            inGP = impg;
            var CharCode;
            if (window.event) {
        ...
        var inGP = 0;
        $scope.onPortCodeChange = function (index) {
            debugger;
            inGP = index;
        }

        $scope.SelectPortOfCall = function (PortOfCallId, PortOfCallName,PortOfCallCode) {
            debugger;                        
            $scope.OblEntryDetails[inGP].GatewayPortCode = PortOfCallCode;
            $('#PortOfCallbox').val('');
