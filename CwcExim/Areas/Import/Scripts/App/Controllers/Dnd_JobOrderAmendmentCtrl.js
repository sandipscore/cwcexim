(function () {
    ...
    controller('JobOrderAmendmentCtrl', function ($scope, JobOrderAmendmentService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        debugger;
        $scope.JODetails = [
        ...
        function GetJobOrderByRoadByOnEditMode() {
            debugger;
            JobOrderAmendmentService.GetJobOrderByRoadByOnEditMode($('#FormOneId').val()).then(function (response) {
                debugger;
        ...
        $scope.ResetImpJODetails = function () {
            ...
            //$('#TransportBy').val('');
            //$scope.JODetails.length = 0;
            $('#DivBody').load('/Import/Dnd_CWCImport/JobOrderByRoadAmendment/');
        }
        ...
        $scope.OnJobOrderSave = function () {
            ...
            debugger;
            if (confirm('Are you sure to save Job Order Amendment ?')) {
                debugger;
                Obj = {
                    ...
                    VesselNo: $('#ViaNo').val(),
                    Remarks: $('#Remarks').val(),
                    IsScanning: $('#IsScanning').val() == false ? 0: 1,
                    ReScanning: $('#ReScanning').val() == false ? 0: 1
                }

                JobOrderAmendmentService.OnJobOrderSave(Obj, JSON.stringify($scope.JODetails)).then(function (res) {
                    //console.log(res);
