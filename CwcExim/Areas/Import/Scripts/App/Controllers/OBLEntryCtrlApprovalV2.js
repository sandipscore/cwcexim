      ...
      //  $(function () {
       ...
       // });
        ...
        $scope.onImporterChange = function (index) {
            debugger;
            inImp = index;
            var name = $('#ImporterName_' + index).val().split('Address')[0];
            $('#Impbox').val(name.substring(0,name.length-1));
        ...
        $scope.OnOBLEntrySave = function () {
          ...
          /*  if (flag1 == 1) {
            ...
            if (confirm('Are you sure to save OBL Entry Approval?')) {
                debugger;
                var isApproved = 0;
                if ($('#Approved').is(":checked") == true)
                {
                    isApproved = 1; //1 For Approved
                }
                else {
                    isApproved = 2; //2 for Rejected
                }

                Obj = {
                    Id: $('#Id').val() == undefined ? 0 : $('#Id').val(),
                    EximHrdId: $('#EximHrdId').val(),
        ...
        function LoadOblEntry() {
            $('#DivBody').load('/Import/Ppg_OblEntryV2/OBLEntryApproval');
        }
