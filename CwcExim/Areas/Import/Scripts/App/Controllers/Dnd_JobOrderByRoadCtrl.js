(function () {
    ...
    controller('JobOrderByRoadCtrl', function ($scope, JobOrderByRoadService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        $scope.JODetails = [
            {
        ...
        function GetJobOrderByRoadByOnEditMode() {
            debugger;
            JobOrderByRoadService.GetJobOrderByRoadByOnEditMode($('#FormOneId').val()).then(function (response) {
                debugger;
        ...
        $scope.OnJobOrderSave = function () {
            ...
            if (flag1 == 1) {
                alert('Please Enter Containe No.');
                return false;
            }
            if (flag2 == 1) {
            ...
            var len = $scope.JODetails.length;
            if (len == 0) {
                alert('At least one record should required for job order details');
                return false;
            }
            debugger;
            if (confirm('Are you sure to save Job Order By Road?')) {
                ...
                JobOrderByRoadService.OnJobOrderSave(Obj, JSON.stringify($scope.JODetails)).then(function (res) {
                    //console.log(res);
