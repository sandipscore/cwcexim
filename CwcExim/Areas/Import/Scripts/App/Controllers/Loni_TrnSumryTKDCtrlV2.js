(function () {
    ...
    .controller('TrainSummaryTKDCtrl', function ($scope, TrainSummaryTKDService, Upload, $timeout) {
        $scope.TrainNo = '';
        $scope.TrainDate = '';
        $scope.PortName = '';
        $scope.PortId = 0;
        ...
        $scope.PopulatePortList = function () {
            $('.modalloader').show();
            TrainSummaryTKDService.PopulatePortList().then(function (res) {
                $('.modalloader').hide();
                $scope.PortList = [];
                if (res.data != null) {
        ...
        $scope.UploadFinal = function () {
            ...
            if ($scope.SelectedFiles && $scope.SelectedFiles.length) {
                ...
                Upload.upload({
                    url: '/Import/Ppg_CWCImportV2/CheckUpload/',
                    data: {
        ...
        $scope.SaveInvoice = function (i) {
            ...
            TrainSummaryTKDService.SaveInvoice($scope.TrainSummaryList[i], $('#SEZ').val()).then(function (response) {
                debugger;
                $('.modalloader').hide();
        ...
        $scope.ViewTrainSummary = function (TrainSummaryUploadId, TrainNo, TrainDate, PortName) {
            ...
            $scope.IsUploaded = 1;
            $('#TrainDate').datepicker("option", "disabled", true);
            TrainSummaryTKDService.ViewTrainSummary(TrainSummaryUploadId).then(function (response) {
                $('.modalloader').hide();
        ...
        $scope.LoadMoreShippingList = function () {
            ...
            TrainSummaryTKDService.LoadMoreShippingList($scope.Page, $scope.PartyCode).then(function (response) {
                $('.modalloader').hide();
        ...
        $scope.GenerateIRN = function () {
            ...
            TrainSummaryTKDService.GenerateIRNNo($scope.InvoiceNo, $scope.SupplyType).then(function (res) {
