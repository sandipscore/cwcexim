(function () {
    ...
    controller('Dnd_OBLWiseContainerEntryCtrl', function ($scope, Dnd_OBLWiseContainerService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        ...
        $scope.ChangeCont = function (index) {
            ...
            ind = index
            ind.ShippingLineName = "";
            ind.ShippingLineId = 0;
            Dnd_OBLWiseContainerService.GetContCBT(index.ContainerNo, index.ContainerSize).then(function (response) {

               // $("#CFSCode").val((index.ContainerNo));
                var html = '';
                //var tabindex = 32;
              
                $.each(response.data, function (item, elem) {
                    debugger;
                    //html += '<tr  onclick="ChangeCont(&quot;' + elem.ContainerNo + '&quot;,&quot;' + elem.ShippingLineId + '&quot;,&quot;' + elem.ShippingLineName + '&quot;)"><td>' + elem.ContainerNo + '</td></tr>';
                    //  tabindex++;
                    if (elem.ContainerNo != ""&&elem.Size != "") {
                        ind.ShippingLineName = elem.ShippingLineName;
                        ind.ShippingLineId = elem.ShippingLineId;
                    }
                });
        ...
        $scope.Ch = function (Cont, Sid, Sname) {
            debugger;
        ...
        $scope.GetOBLDetails = function () {
            ...
            Dnd_OBLWiseContainerService.GetOBLDetails(OBLNo).then(function (response) {
                debugger;
                $scope.OblEntryDetails = [];
        ...
        $scope.OnOBLEntrySave = function () {
            ...
            if ($('#SMTPNo').val() == null || $('#SMTPNo').val() == '') {
                alert('Please Enter SMTP No');
                return false;
            }
            if ($('#SMTP_Date').val() == null || $('#SMTP_Date').val() == '') {
                alert('Please Enter SMTP Date');
                return false;
            }
         
            if ($('#CargoDescription').val() == null || $('#CargoDescription').val() == '') {
            ...
            if ($('#NoOfPkg').val() == 0) {
                alert('Please Enter NoOfPkg');
                return false;
            }

            if ($('#PkgType').val() == '') {
                alert('Please Enter PkgType');
                return false;
            }
            if ($('#GR_WT').val() == 0) {
            ...
            if (confirm('Are you sure to save OBL Entry?')) {
                ...
                Obj = {
                    ...
                    IsITP: $('#IsITP').is(':checked') == true ? 1 : 0,
                }
                Dnd_OBLWiseContainerService.OBLEntrySave(Obj, JSON.stringify($scope.OblEntryDetails)).then(function (res) {
                    console.log(res);
        ...
        function LoadOblEntry() {
            $('#DivBody').load('/Import/Dnd_OblEntry/OBLWiseContainerEntry');
        }
