    ...
    controller('OBLWiseContainerEntryCtrl', function ($scope, OBLWiseContainerService) {
        ...
        $scope.GetOBLDetails = function () {
            ...
            OBLWiseContainerService.GetOBLDetails(OBLNo).then(function (response) {
                ...
                $('#ICEGateImporterName').val(response.data.ICEGateImporterName);
        ...
        $scope.OnOBLEntrySave = function () {
            ...
            if ($('#Approved').is(":checked") == false && $('#Rejected').is(":checked") == false) {
                alert('Please Check Approval Status.');
                return false;
            }
        ...
        function LoadOblEntry() {
            $('#DivBody').load('/Import/Loni_OblEntryV2/OblEntryFCLApproval');
        }
        ...
        $scope.GetContainerNo = function () {
            debugger;
            OBLWiseContainerService.OBLEntryDetails().then(function (res) {
                $scope.LstContainerNo = res.data;
        ...
        $scope.GetOBlDetailsByOblNo = function (id) {
            $('#OblDetailsId').val(id);
            debugger
            OBLWiseContainerService.OBLEntryDetailsByObl(id).then(function (res) {

                $scope.ContainerDetails = res.data;
                $('#OBL_No').val($scope.ContainerDetails.OBL_No);
                $('#OBL_Date').val($scope.ContainerDetails.OBL_Date);
                $('#LineNo').val($scope.ContainerDetails.LINE_NO);
                $('#SMTPNo').val($scope.ContainerDetails.SMTPNo);
                $('#SMTP_Date').val($scope.ContainerDetails.SMTP_Date);
                $('#NoOfPkg').val($scope.ContainerDetails.NoOfPkg);
                $('#CargoType').val($scope.ContainerDetails.CargoType == '0' ? '' : $scope.ContainerDetails.CargoType);
                $('#CargoDescription').val($scope.ContainerDetails.CargoDescription);
                $('#PkgType').val($scope.ContainerDetails.PkgType);
                $('#NoOfPkg').val($scope.ContainerDetails.NoOfPkg);
                $('#GR_WT').val($scope.ContainerDetails.GR_WT);
                $('#ImporterId').val($scope.ContainerDetails.ImporterId);
                $('#ImporterName').val($scope.ContainerDetails.ImporterName);
                $('#IGM_No').val($scope.ContainerDetails.IGM_No);
                $('#IGM_Date').val($scope.ContainerDetails.IGM_Date);
                $('#TPNo').val($scope.ContainerDetails.TPNo);
                $('#TPDate').val($scope.ContainerDetails.TPDate);
                $('#MovementType').val($scope.ContainerDetails.MovementType);
                $('#ICEGateImporterName').val($scope.ContainerDetails.ICEGateImporterName);

                $('#Commodity').val($scope.ContainerDetails.Commodity);
                $('#CommodityId').val($scope.ContainerDetails.CommodityId);
                $('#CountryId').val($scope.ContainerDetails.CountryId);
                $('#PortId').val($scope.ContainerDetails.PortId);
                ...
                var j = 0;
                angular.forEach($scope.ContainerDetails.OblEntryDetailsList, function (item) {
                    angular.forEach($scope.OblEntryDetails, function (item1) {
                        if (item1.ContainerNo == item.ContainerNo) {
                            j = 1;
                        }
