    ...
    controller('EmpContTrnsCtrl', function ($scope, EmpContTrnsService) {
        ...
        $scope.SelectContainer = function (item) {
            ...
            EmpContTrnsService.GetEmptyContToShipLineForTransfer(item.ContainerNo).then(function (res) {
                ...
                if (res.data.length > 0) {
                    ...
                    $scope.GstNo = res.data[0].GstNo;
                    $scope.PlaceOfSupply = res.data[0].State;
                }
                else {
                    ...
                    $scope.PlaceOfSupply = item.StateName;
                }
               });
            debugger;
            $scope.c = 1;
        ...
        $scope.CalculateCharges = function () {
            ...
            EmpContTrnsService.GetEmptyContTransferCharges($scope.InvoiceDate, TaxType, $scope.CFSCode, $scope.ContainerNo, $scope.Size, $scope.EntryDateTime,
               $scope.DestuffDate, $scope.RefId, $scope.FromShippingLineId, $scope.PayeeId, 0, $scope.SEZ).then(function (res) {
        ...
        $scope.Reset = function () {
            $('#DivBody').load('/Import/Ppg_CwcImport/EmptyContainerTransferInv?type=Tax');
        }
        ...
        $scope.GenerateIRN = function () {
            ...
            EmpContTrnsService.GenerateIRNNo($scope.InvoiceNo, $scope.SupplyType).then(function (res) {
