(function () {
    ...
    controller('DSRRCTSealingInvoiceCtrl', function ($scope, DSRRCTSealingInvoiceService) {
        ...
        $scope.CreateRCTSealingInvoice = function () {
            ...
            if ($('#OBLDetailsJson').val() != "")
            {
                $scope.OBLDetails = JSON.parse($('#OBLDetailsJson').val());
            }
            else {
                $scope.OBLDetails = null;
            }
            ...
            //if (c > 0) {
            DSRRCTSealingInvoiceService.GetRCTSealingInvoice(0, $('#InvoiceDate').val(), TaxType, $scope.ConatinerDetails, $scope.OBLDetails, $scope.PartyId, $scope.PayeeId,$scope.SEZ).then(function (res) {
