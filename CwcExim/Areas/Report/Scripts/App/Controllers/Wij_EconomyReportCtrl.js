(function () {
    angular.module('CWCApp')
        ...
        .controller('Wij_EconomyReportCtrl', function ($scope, Wij_EconomyReportSvc) {
            $scope.Msg = '';
            $scope.months = [
                { "Value": "0", "Text": "--Select--" },
                { "Value": "1", "Text": "Jan" },
            ...
            $scope.GetEconomyReport = function () {
                Wij_EconomyReportSvc.GetEconomyReport($scope.Month, $scope.Year).then(function (res) {
            ...
            $scope.CalculateSum = function () {
                ...
                var total = parseFloat($scope.EcoRptData[$scope.indexOfIncomeExpHeadId(67)].Amount);
             ...
             //   var rail = parseFloat($scope.EcoRptData[$scope.indexOfIncomeExpHeadId(150)].Amount);
                ...
                var isum = impcargo + impgr + expcargo + expgr + ht + auc + misc + adv+total ;
             ...
             //   $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(67)].Amount = isum.toString();
                ...
                var totalincome = parseFloat($scope.EcoRptData.filter(x=>x.IncomeExpHeadId == 68)[0].Amount); //Total Income

                $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(156)].Amount = s.toString(); //Total Expense
                if ((totalincome - s) > 0) {
                 //   $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(69)].Amount = (totalincome - s).toString(); //Total Income-Total Expense
                    $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(158)].Amount = (totalincome - s).toString(); //Total Income-Total Expense
                }

                else

                {
                    $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(157)].Amount = (s - totalincome).toString(); //Total Income-Total Expense
            ...
            $scope.SaveEcoRpt = function () {
                ...
                if (confirm('Are You Sure To Save?')) {
                    debugger;
                    Wij_EconomyReportSvc.SaveEconomyReport($scope.Month, $scope.Year, $scope.EcoRptData).then(function (res) {
                        console.log(res.data.Data);
                        if (res.data.Status == 1) {
