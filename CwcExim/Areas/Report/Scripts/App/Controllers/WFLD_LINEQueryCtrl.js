(function () {
    ...
    controller('WFLD_LINEQueryCtrl', function ($scope, WFLD_LINEQueryService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        ...
        $scope.GetOBLDetailsForOBLQuery = function () {
            ...
            WFLD_LINEQueryService.GetLINEDetailsForLINEQuery(LINENo,OBLNo, SearchBy).then(function (response) {
                debugger;
                $scope.OblEntryDetails = [];
                if (response.data.OblEntryDetailsList.length == 0) {
                    alert('No record found for given LINE No.');
                    return false;
                }
