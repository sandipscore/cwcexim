(function () {
    angular.module('CWCApp')
        ...
        .controller('HdbEconomyReportCtrl', function ($scope, HdbEconomyReportSvc) {
        $scope.Msg = '';
        $scope.months = [
            { "Value": "0", "Text": "--Select--" },
            { "Value": "1", "Text": "Jan" },
        ...
        $scope.GetEconomyReport = function () {
            HdbEconomyReportSvc.GetEconomyReport($scope.Month, $scope.Year).then(function (res) {
          ...
          //  var rail = parseFloat($scope.EcoRptData[$scope.indexOfIncomeExpHeadId(150)].Amount);
           ...
           // $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(148)].Amount = s.toString(); //Total Expense
            ...
            if ((totalincome - s) >= 0) {
                ...
                $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(149)].Amount = ""; //Total Income-Total Expense
            }
            else {
                $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(149)].Amount = (s - totalincome).toString();
                $scope.EcoRptData[$scope.indexOfIncomeExpHeadId(69)].Amount = "";
            }
        ...
        $scope.SaveEcoRpt = function () {
            ...
            if (confirm('Are You Sure To Save?')) {
                debugger;
                HdbEconomyReportSvc.SaveEconomyReport($scope.Month, $scope.Year, $scope.EcoRptData).then(function (res) {
                    console.log(res.data.Data);
                    if (res.data.Status == 1) {
