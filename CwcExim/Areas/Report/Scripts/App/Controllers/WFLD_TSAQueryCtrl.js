(function () {
    ...
    controller('WFLD_TSAQueryCtrl', function ($scope, WFLD_TSAQueryService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        ...
        $scope.GetOBLDetailsForOBLQuery = function () {
            debugger;
            var TSANo = $('#TSANo').val();
            var OBLNo = $('#OBLNo').val();
          //  var obl = OBLNo.split('-');
         ...
         //   var obldate = obl[1];
            ...
            WFLD_TSAQueryService.GetTSADetailsForTSAQuery(TSANo,OBLNo, SearchBy).then(function (response) {
                debugger;
                $scope.OblEntryDetails = [];
                if (response.data.OblEntryDetailsList.length == 0) {
                    alert('No record found for given TSA No.');
                    return false;
                }
                ...
                $('#InvNo').val(response.data.InvNo);
                $('#InvDate').val(response.data.InvDate);
                $('#GatePassNo').val(response.data.GatePassNo);
                $('#GatePassDate').val(response.data.GatePassDate);
                $('#GateExitNo').val(response.data.GateExitNo);
                $('#GateExitDate').val(response.data.GateExitDate);
                $('#TSANo').val(response.data.TSANo);
                $('#TSADate').val(response.data.TSADate);
                $('#GodownNo').val(response.data.GodownNo);
                $('#Location').val(response.data.Location);
                $('#LineNo').val(response.data.LineNo);
                $('#CBM').val(response.data.CBM);
                $('#SQM').val(response.data.SQM);
                $('#ShippingLine').val(response.data.ShippingLine);
                $('#DestuffingNo').val(response.data.DestuffingNo);
                $('#DestuffingDate').val(response.data.DestuffingDate);
        ...
        $scope.ResetDet = function () {
            ...
            $('#DivBody').load('/Report/WFLD_ReportCWC/TSAGateDetail?SearchBy=' + prm);
