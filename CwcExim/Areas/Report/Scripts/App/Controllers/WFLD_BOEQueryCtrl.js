(function () {
    ...
    controller('WFLD_BOEQueryCtrl', function ($scope, WFLD_BOEQueryService) {
        $scope.InvoiceNo = "";
        var t2 = 0;
        ...
        $scope.GetBOEDetailsForBOEQuery = function () {
            debugger;
            var OBLNo = $('#BOENo').val();
            //  var obl = OBLNo.split('-');
            // var oblnum = obl[0];
            ...
            WFLD_BOEQueryService.GetBOEDetailsForBOEQuery(OBLNo, SearchBy).then(function (response) {
                debugger;
                $scope.OblEntryDetails = [];
                if (response.data.OblEntryDetailsList.length == 0) {
                    alert('No record found for given BOE No.');
                    return false;
                }
