...
{
    public class Hdb_PestRepository
    {
        ...
        public void AddEditPestControl(Hdb_PestControl ObjPostPaymentSheet, String ChemicalData, int BranchId, int Uid, string Module)
        {
            ...
            LstParam.Add(new MySqlParameter { ParameterName = "in_HTClause", MySqlDbType = MySqlDbType.VarChar, Value = ObjPostPaymentSheet.HTClause });
            LstParam.Add(new MySqlParameter { ParameterName = "in_HTClauseId", MySqlDbType = MySqlDbType.VarChar, Value = ObjPostPaymentSheet.HTClauseId });
        ...
        public void ListOfAllHTClause()
        {
            int Status = 0;
            DataAccessLayerBaseClass DataAccess = DataAccessLayerFactory.GetDataAccessLayer();
            IDataReader Result = DataAccess.ExecuteDataReader("ListOfAllHTClause", CommandType.StoredProcedure);
            ...
            try
            {
                ...
                {
                    Status = 1;
                    LstPest.Add(new Hdb_PestControl
                    {
                        HTClause = Convert.ToString(Result["OperationCode"]),
                        HTClauseId = Convert.ToInt32(Result["OperationId"]),
                    });
                }
                if (Status == 1)
                {
