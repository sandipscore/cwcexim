...
{
    public class Wfld_IceGateRepository
    {
            ...
            try
            {
                ...
                foreach(DataRow dr in Result.Tables[1].Rows)
                {
                    lstTP.Add(new {
                        ...
                        OriginPortCode = dr["OriginPortCode"].ToString(),
                        ContainerNo = Convert.ToString(dr["ContainerNo"]),
                        Size = Convert.ToString(dr["Size"]),
                        //PortName = dr["PortName"].ToString(),
                        GatewayPortCode = dr["GatewayPortCode"].ToString(),
                        SLA = Convert.ToString(dr["ShippingLineCode"])
                    });
                }
                if (Status == 1)
                {
