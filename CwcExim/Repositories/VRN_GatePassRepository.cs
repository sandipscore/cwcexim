...
{
    public class VRN_GatePassRepository
    {
        ...
        /***************************/
        public void AddEditGatePass(VRN_GatePass objGP, string XML, string VehicleXml)
        {
            ...
            try
            {
                ...
                {
                    _DBResponse.Status = Result;
                    _DBResponse.Message = "Cash Party Amount Not Paid Yet or SD Party Balance is low";
                    _DBResponse.Data = null;
                }
                else if (Result == 4)
                {
                    _DBResponse.Status = Result;
                    _DBResponse.Message = "Gate Pass Cannot Be Updated As It Already Exists In  Exit Through Gate";
                    _DBResponse.Data = null;
                }
                else
                {
        ...
        public void GetDetailsForGatePassPrint(int GatePassId)
        {
            ...
            try
            {
                ...
                if (Result.NextResult())
                {
                    while (Result.Read())
                    {
                        objGP.lstDet.Add(new VRN_GPDet
                        {
                            ContainerNo = Result["ContainerNo"].ToString(),
                ...
                if (Result.NextResult())
                {
                    while (Result.Read())
                    {
                        objGP.lstVehicle.Add(new VRN_GatepassVehicle
                        {
                            ContainerNo = Result["ContainerNo"].ToString(),
                            VehicleNo = (Result["VehicleNo"] == null ? "" : Result["VehicleNo"]).ToString(),
        ...
        public void GetDetForGatePass(int GatePassId)
        {
            ...
            try
            {
                ...
                if (Result.NextResult())
                {
                    List<VRN_GatepassVehicle> lstVch = new List<VRN_GatepassVehicle>();
                    while (Result.Read())
                    {
                        lstVch.Add(new VRN_GatepassVehicle
                        {
