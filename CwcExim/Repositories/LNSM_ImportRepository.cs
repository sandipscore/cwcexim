...
{
    //For SRS Version 3.2 
    public class LNSM_ImportRepository
    {
        ...
        #region Train Summary LONI SAMO
        public void AddUpdateTrainSummaryUploadLONISAMO(string TrainSummaryUploadXML)
        {
            try
            {
                string GeneratedClientId = "";
                ...
                DParam = LstParam.ToArray();
                DataAccessLayerBaseClass DataAccess = DataAccessLayerFactory.GetDataAccessLayer();
                int Result = DataAccess.ExecuteNonQuery("AddEditTrnSmaryUpldLoniSamo", CommandType.StoredProcedure, DParam, out GeneratedClientId);
                _DBResponse = new DatabaseResponse();

                if (Result == 1)
                {
                    _DBResponse.Status = 1;
                    _DBResponse.Message = "Train summary uploaded successfully.";
                    _DBResponse.Data = GeneratedClientId;
                }               
                else
                {
        ...
        public void CheckTrainSummaryUploadLONISAMO(string TrainNo, string TrainSummaryUploadXML, string TrainDate)
        {
            DataSet Result = new DataSet();
            try
            {
                ...
                DParam = LstParam.ToArray();
                DataAccessLayerBaseClass DataAccess = DataAccessLayerFactory.GetDataAccessLayer();
                Result = DataAccess.ExecuteDataSet("checkTrnSmaryUpldLoniSamo", CommandType.StoredProcedure, DParam);
                _DBResponse = new DatabaseResponse();
                ...
                foreach (DataRow dr in Result.Tables[0].Rows)
                {
                    ...
                    objTrainSummaryUpload.SrNo = TrainSummaryUploadList.Count + 1;
                    objTrainSummaryUpload.TrainNo = TrainNo;
                    objTrainSummaryUpload.TrainDate = TrainDate;
                    objTrainSummaryUpload.NormalHub = Convert.ToString(dr["ContType"]);
                    objTrainSummaryUpload.Wagon_No = Convert.ToString(dr["WagonNo"]);
                    objTrainSummaryUpload.Stack = Convert.ToString(dr["Stack"]);
                    objTrainSummaryUpload.Container_No = Convert.ToString(dr["Container_No"]);
                    objTrainSummaryUpload.LE = Convert.ToString(dr["LE"]);
                    objTrainSummaryUpload.CT_Size = Convert.ToString(dr["CT_Size"]);
                    objTrainSummaryUpload.Cargo = Convert.ToString(dr["Cargo"]);
                    objTrainSummaryUpload.Cargo_Wt = Convert.ToDecimal(dr["Cargo_Wt"]);
                    objTrainSummaryUpload.Ct_Tare = Convert.ToDecimal(dr["Ct_Tare"]);
                    objTrainSummaryUpload.Total_Wt = Convert.ToDecimal(dr["Gross_Wt"]);
                    objTrainSummaryUpload.Exim = Convert.ToString(dr["operationType"]);
                    objTrainSummaryUpload.StartLocation = Convert.ToString(dr["LocationFrom"]);
                    objTrainSummaryUpload.Destination = Convert.ToString(dr["Destination"]);                    
                    objTrainSummaryUpload.Remarks = Convert.ToString(dr["Remarks"]);   
                    TrainSummaryUploadList.Add(objTrainSummaryUpload);
                }
        ...
        public void GetTrainSummaryDetailsLONISAMO(int TrainSummaryUploadId)
        {
            DataSet Result = new DataSet();
            int Status = 0;
            try
            {
                ...
                DParam = LstParam.ToArray();
                Result = DataAccess.ExecuteDataSet("GetTrainSummaryDetailsLONISAMO", CommandType.StoredProcedure, DParam);
                ...
                if (Result != null && Result.Tables.Count > 0)
                {
                    foreach (DataRow dr in Result.Tables[0].Rows)
                    {
                        ...
                        LNSM_TrainSummaryUpload objTrainSummaryUpload = new LNSM_TrainSummaryUpload();
                        objTrainSummaryUpload.SrNo = TrainSummaryUploadList.Count + 1;
                        objTrainSummaryUpload.TrainNo = Convert.ToString(dr["TrainNumber"]); ;
                        objTrainSummaryUpload.TrainDate = Convert.ToString(dr["Movementdate"]); ;
                        objTrainSummaryUpload.NormalHub = Convert.ToString(dr["ContType"]);
                        objTrainSummaryUpload.Wagon_No = Convert.ToString(dr["WagonNo"]);
                        objTrainSummaryUpload.Stack = Convert.ToString(dr["Stack"]);
                        objTrainSummaryUpload.Container_No = Convert.ToString(dr["ContNumber"]);
                        objTrainSummaryUpload.LE = Convert.ToString(dr["LE"]);
                        objTrainSummaryUpload.CT_Size = Convert.ToString(dr["Size"]);
                        objTrainSummaryUpload.Cargo = Convert.ToString(dr["Cargo"]);
                        objTrainSummaryUpload.Cargo_Wt = Convert.ToDecimal(dr["CargoWt"]);
                        objTrainSummaryUpload.Ct_Tare = Convert.ToDecimal(dr["TareWt"]);
                        objTrainSummaryUpload.Total_Wt = Convert.ToDecimal(dr["TotalWt"]);
                        objTrainSummaryUpload.Exim = Convert.ToString(dr["operationType"]);
                        objTrainSummaryUpload.StartLocation = Convert.ToString(dr["FromLocation"]);
                        objTrainSummaryUpload.Destination = Convert.ToString(dr["ToLocation"]);
                        objTrainSummaryUpload.Remarks = Convert.ToString(dr["Remarks"]);
                        TrainSummaryUploadList.Add(objTrainSummaryUpload);
                    }
                }
        ...
        public void ListOfTrainSummaryLONISAMO()
        {
            DataSet Result = new DataSet();
            int Status = 0;
            try
            {
                DataAccessLayerBaseClass DataAccess = DataAccessLayerFactory.GetDataAccessLayer();
                Result = DataAccess.ExecuteDataSet("GetTrainSummaryListLoniSamo", CommandType.StoredProcedure);
                _DBResponse = new DatabaseResponse();
