...
{
    public class VLDA_CountryRepository
    {
        ...
        //Get Country By Port of Destination ID
        public void GetCountryByDestinationID(int DestinationID)
        {
            int Status = 0;
            List<MySqlParameter> LstParam = new List<MySqlParameter>();
            LstParam.Add(new MySqlParameter { ParameterName = "in_PodId", MySqlDbType = MySqlDbType.Int32, Size = 11, Value = DestinationID });
            IDataParameter[] DParam = { };
            DParam = LstParam.ToArray();
            DataAccessLayerBaseClass DataAccess = DataAccessLayerFactory.GetDataAccessLayer();
            IDataReader Result = DataAccess.ExecuteDataReader("GetCountryNameByPOD", CommandType.StoredProcedure, DParam);
            _DBResponse = new DatabaseResponse();
            Country ObjCountry = new Country();
            try
            {
